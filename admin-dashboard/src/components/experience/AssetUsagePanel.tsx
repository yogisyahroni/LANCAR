...
export default function AssetUsagePanel({ usages }: { usages: AssetUsage[] }) {
  ...
  return <div className="space-y-2">{usages.map((usage) => <article key={`${usage.manifest_id}-${usage.revision}-${usage.surface}`} className="rounded-xl border border-border bg-surface-subtle p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-mono text-xs text-foreground-muted">{usage.manifest_id} · r{usage.revision}</p><StatusBadge status={usage.state} labelPrefix="Manifest state" label={stateLabel[usage.state]} /></div><p className="mt-2 text-xs text-foreground-muted">{usage.market_code} · {usage.surface} · {usage.placements.length ? usage.placements.join(', ') : 'unclassified placement'}</p><p className="mt-1 text-xs text-foreground-muted">{dateLabel(usage.starts_at)} → {dateLabel(usage.ends_at)}</p></article>)}</div>
}
