...
export default function RevisionDiffPreview({ result }: Props) {
  ...
  return (
    <section className="space-y-4 rounded-3xl border border-border bg-surface/[0.03] p-5" aria-labelledby="revision-diff-title">
      ...
      {result.diff.field_changes.length ? <div className="space-y-2 rounded-2xl border border-border bg-surface-subtle p-4" aria-label="field-level revision diff"><p className="text-xs font-black uppercase tracking-wide text-foreground-muted">Field-level changes</p>{result.diff.field_changes.map((change) => <div key={change.field} className="grid gap-2 rounded-xl border border-border bg-surface/[0.02] p-3 text-xs md:grid-cols-[7rem_1fr_1fr] md:items-start"><span className="font-black uppercase tracking-wider text-primary-light">{change.field}</span><span className="break-words text-error"><strong className="mr-1 text-xs uppercase tracking-wide text-foreground-muted">Previous</strong>{valueLabel(change.previous)}</span><span className="break-words text-success"><strong className="mr-1 text-xs uppercase tracking-wide text-foreground-muted">New</strong>{valueLabel(change.next)}</span></div>)}</div> : null}
    </section>
  )
}
