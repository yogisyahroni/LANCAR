          ...
          <div key={`${entry.service_code}-${index}`} className="rounded-xl border border-border bg-surface-subtle p-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-1">
                ...
                <button type="button" disabled={disabled || entries.length <= 1} onClick={() => onChange(resequence(entries.filter((_, itemIndex) => itemIndex !== index)))} className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-bold text-on-error hover:bg-error-surface hover:text-error disabled:opacity-60" aria-label={`Remove ${entry.service_code || 'service'}`} title={`Remove ${entry.service_code || 'service'}`}><Trash2 size={14} aria-hidden="true" /><span>Remove</span></button>
              </div>
            </div>
