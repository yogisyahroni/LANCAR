...
export default function TargetingSummary({ form, conflicts, scheduleOnly = false }: Props) {
  ...
  return (
    <section className="space-y-4 rounded-3xl border border-primary/20 bg-primary/[0.04] p-5" aria-labelledby="targeting-summary-title">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="targeting-summary-title" className="text-sm font-black uppercase tracking-wider text-foreground-muted">{scheduleOnly ? 'Schedule & audience safety' : 'Audience resolution summary'}</h2><p className="mt-1 text-xs leading-relaxed text-foreground-muted">The server resolves targeting and schedule. The customer client receives the selected sanitized manifest, not these rule details.</p></div><StatusBadge status="protected" label="Server-side resolver" labelPrefix="Targeting resolution" className="border-info bg-info-surface text-info" /></div>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-border bg-surface-subtle p-3 text-xs"><p className="font-black uppercase tracking-wide text-foreground-muted">Market</p><p className="mt-1 text-foreground-muted">{list(targeting.market_codes) === 'All' ? form.market_code : list(targeting.market_codes)}</p></div>
        <div className="rounded-2xl border border-border bg-surface-subtle p-3 text-xs"><p className="font-black uppercase tracking-wide text-foreground-muted">City / zone</p><p className="mt-1 text-foreground-muted">{list(targeting.city_codes)} / {list(targeting.zone_codes)}</p></div>
        <div className="rounded-2xl border border-border bg-surface-subtle p-3 text-xs"><p className="font-black uppercase tracking-wide text-foreground-muted">Locale / role</p><p className="mt-1 text-foreground-muted">{list(targeting.locales)} / {list(targeting.roles)}</p></div>
        <div className="rounded-2xl border border-border bg-surface-subtle p-3 text-xs"><p className="font-black uppercase tracking-wide text-foreground-muted">Rollout</p><p className="mt-1 text-foreground-muted">{form.rollout_stage} · {form.rollout_percentage}%{form.canary_cohort ? ` · ${form.canary_cohort}` : ''}</p></div>
      </div>
