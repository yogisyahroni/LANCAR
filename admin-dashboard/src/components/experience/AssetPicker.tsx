...
export default function AssetPicker({ value, onChange, disabled = false, focusAssetId = null }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-surface-subtle p-5" aria-labelledby="asset-picker-title">
      ...
      {value.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-border p-4 text-xs text-foreground-muted">No assets. Components without media can be published safely.</p> : <div className="mt-4 space-y-3">
        {value.map((asset, index) => <div key={`${asset.asset_id}-${index}`} className={`rounded-xl border p-3 ${asset.asset_id === focusAssetId ? 'border-accent bg-accent-surface' : 'border-border bg-surface/[0.03]'}`}>
          ...
          <div className="grid gap-3 md:grid-cols-[1fr_2fr_130px_1fr_auto]">
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">ID<input className={inputClass} disabled={disabled} value={asset.asset_id} onChange={(event) => update(index, { asset_id: event.target.value })} /></label>
            ...
            <button type="button" disabled={disabled} aria-label={`Remove ${asset.asset_id || 'asset'}`} title={`Remove ${asset.asset_id || 'asset'}`} onClick={() => onChange(value.filter((_, assetIndex) => assetIndex !== index))} className="mt-5 self-start rounded-lg p-2 text-on-error transition hover:bg-error-surface hover:text-error disabled:opacity-60"><Trash2 size={16} aria-hidden="true" /></button>
          </div>
          <div className="mt-3 grid gap-3 md:grid-cols-6">
            ...
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">Height<input type="number" min="1" max="4096" className={inputClass} disabled={disabled} value={asset.height ?? ''} onChange={(event) => update(index, { height: optionalNumber(event.target.value) })} /></label>
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">Aspect ratio<input type="number" min="0.1" max="20" step="0.01" className={inputClass} disabled={disabled} value={asset.aspect_ratio ?? ''} onChange={(event) => update(index, { aspect_ratio: optionalNumber(event.target.value) })} placeholder="1.78" /></label>
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">Max bytes<input type="number" min="1" max={MAX_ASSET_BYTES} className={inputClass} disabled={disabled} value={asset.size_limit_bytes ?? MAX_ASSET_BYTES} onChange={(event) => update(index, { size_limit_bytes: Number(event.target.value) })} /></label>
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">Version<input className={inputClass} disabled={disabled} value={asset.version || '1'} onChange={(event) => update(index, { version: event.target.value })} /></label>
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">Cache<select className={inputClass} disabled={disabled} value={asset.cache_policy || 'private'} onChange={(event) => update(index, { cache_policy: event.target.value as ExperienceAsset['cache_policy'] })}><option value="private">private</option><option value="public">public</option><option value="no-store">no-store</option></select></label>
          </div>
          <div className="mt-3 grid gap-3 md:grid-cols-[2fr_2fr_1fr]">
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">SHA-256<input className={inputClass} disabled={disabled} value={asset.checksum} onChange={(event) => update(index, { checksum: event.target.value })} placeholder="64 hex characters" /></label>
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">Fallback asset ID<input className={inputClass} disabled={disabled} value={asset.fallback_asset_id || ''} onChange={(event) => update(index, { fallback_asset_id: event.target.value.trim() || null })} placeholder="lighter-webp" /></label>
            <label className="text-[10px] font-bold uppercase tracking-wider text-foreground-muted">Expires at<input type="datetime-local" className={inputClass} disabled={disabled} value={toDateTimeInput(asset.expires_at)} onChange={(event) => update(index, { expires_at: toIsoDate(event.target.value) })} /></label>
          </div>
