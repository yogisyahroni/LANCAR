      ...
      {value.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-border p-4 text-xs text-foreground-muted">No assets. Components without media can be published safely.</p> : <div className="mt-4 space-y-3">
        {value.map((asset, index) => <div key={`${asset.asset_id}-${index}`} className={`rounded-xl border p-3 ${asset.asset_id === focusAssetId ? 'border-accent bg-accent-surface' : 'border-border bg-surface/[0.03]'}`}>
          ...
          <div className="grid gap-3 md:grid-cols-[1fr_2fr_130px_1fr_auto]">
            ...
            <button type="button" disabled={disabled} aria-label={`Remove ${asset.asset_id || 'asset'}`} title={`Remove ${asset.asset_id || 'asset'}`} onClick={() => onChange(value.filter((_, assetIndex) => assetIndex !== index))} className="mt-5 inline-flex self-start items-center justify-center gap-1 rounded-lg px-2 py-2 text-xs font-bold text-on-error transition hover:bg-error-surface hover:text-error disabled:opacity-60"><Trash2 size={16} aria-hidden="true" /><span>Remove</span></button>
          </div>
          <div className="mt-3 grid gap-3 md:grid-cols-6">
            <label className="text-xs font-bold tracking-wide text-foreground-muted">Width<input type="number" min="1" max="4096" className={inputClass} disabled={disabled} value={asset.width ?? ''} onChange={(event) => update(index, { width: optionalNumber(event.target.value) })} /></label>
            <label className="text-xs font-bold tracking-wide text-foreground-muted">Height<input type="number" min="1" max="4096" className={inputClass} disabled={disabled} value={asset.height ?? ''} onChange={(event) => update(index, { height: optionalNumber(event.target.value) })} /></label>
            <label className="text-xs font-bold tracking-wide text-foreground-muted">Aspect ratio<input type="number" min="0.1" max="20" step="0.01" className={inputClass} disabled={disabled} value={asset.aspect_ratio ?? ''} onChange={(event) => update(index, { aspect_ratio: optionalNumber(event.target.value) })} placeholder="1.78" /></label>
            <label className="text-xs font-bold tracking-wide text-foreground-muted">Max bytes<input type="number" min="1" max={MAX_ASSET_BYTES} className={inputClass} disabled={disabled} value={asset.size_limit_bytes ?? MAX_ASSET_BYTES} onChange={(event) => update(index, { size_limit_bytes: Number(event.target.value) })} /></label>
            <label className="text-xs font-bold tracking-wide text-foreground-muted">Version<input className={inputClass} disabled={disabled} value={asset.version || '1'} onChange={(event) => update(index, { version: event.target.value })} /></label>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-[2fr_2fr_1fr]">
            <label className="text-xs font-bold tracking-wide text-foreground-muted">SHA-256<input className={inputClass} disabled={disabled} value={asset.checksum} onChange={(event) => update(index, { checksum: event.target.value })} placeholder="64 hex characters" /></label>
