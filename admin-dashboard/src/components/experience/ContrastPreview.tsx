    ...
    <section className="rounded-3xl border border-border bg-surface/[0.03] p-5" aria-labelledby="contrast-preview-title">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="rounded-2xl border border-border p-5" style={{ backgroundColor: preview.background, color: preview.text }}>
          <div className="flex items-center justify-between gap-3"><span className="text-xs font-black uppercase tracking-wide">{modeLabels[mode]} preview · {effectiveMode}</span><span className="rounded-full px-2 py-1 text-xs font-black" style={{ backgroundColor: preview.accent, color: preview.accentText }}>Badge</span></div>
          <div className="mt-5 rounded-2xl p-4" style={{ backgroundColor: preview.accent, color: preview.accentText }}><p className="text-sm font-black">Campaign card</p><p className="mt-1 text-xs opacity-90">Presentation tokens affect this campaign surface only.</p><button type="button" className="mt-4 rounded-xl px-3 py-2 text-xs font-black uppercase tracking-wide" style={{ backgroundColor: preview.background, color: preview.text }}>Safe CTA preview</button></div>
          <p className="mt-4 text-xs">Transaction screens retain packaged theme values.</p>
        </div>
        <div className="space-y-2">
          {validation.checks.map((check) => <div key={check.id} className={`rounded-xl border p-3 ${check.pass ? 'border-success bg-success/[0.06]' : 'border-error bg-error/[0.06]'}`}><div className="flex items-center gap-2 text-xs font-black">{check.pass ? <CheckCircle2 size={14} aria-hidden="true" className="text-success" /> : <XCircle size={14} aria-hidden="true" className="text-error" />}<span className={check.pass ? 'text-success' : 'text-error'}>{check.label}</span></div><p className="mt-1 text-xs text-foreground-muted">{check.ratio.toFixed(2)}:1 · minimum 4.50:1</p></div>)}
