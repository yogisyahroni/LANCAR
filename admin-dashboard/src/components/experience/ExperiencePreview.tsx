      ...
      <div className="mt-5 space-y-3 rounded-2xl bg-zinc-900 p-4">
        {visibleSections.length === 0 ? <p className="p-8 text-center text-sm text-zinc-600">Add an enabled approved component to preview it.</p> : visibleSections.map((section) => {
          ...
          return <article key={section.id} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
            ...
            <div className="p-4">{section.component === 'campaign_intro' ? <div className="mb-3 rounded-xl border border-sky-500/20 bg-sky-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-sky-200">Post-native-splash campaign intro · not the OS launch splash</div> : null}<div className="mb-2 flex items-center justify-between gap-3"><span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">{section.component}</span>{text(properties, 'badge') ? <span className="rounded-full bg-primary/15 px-2 py-1 text-[10px] font-black text-primary-light">{text(properties, 'badge')}</span> : null}</div><h3 className="text-base font-black text-zinc-100">{title}</h3>{body ? <p className="mt-2 text-sm leading-relaxed text-zinc-400">{body}</p> : null}{cta ? <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-black text-white">{cta}<ExternalLink size={13} /></div> : null}</div>
          </article>
        })}
      </div>
