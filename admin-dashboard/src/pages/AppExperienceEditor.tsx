...
export default function AppExperienceEditor({
  ...
  const renderPromoItems = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-border bg-surface-subtle p-3">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-foreground-muted">Promo items</p><p className="mt-1 text-[11px] text-foreground-muted">Presentation only; campaign eligibility is resolved by Promo/Pricing.</p></div><button type="button" disabled={disabled || items.length >= 10} onClick={() => addArrayItem(sectionIndex, 'items')} className="inline-flex items-center gap-1 rounded-lg border border-primary/30 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-primary-light disabled:opacity-50"><Plus size={12} aria-hidden="true" /> Add item</button></div>
  ...
  const renderQuickActions = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-border bg-surface-subtle p-3">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-foreground-muted">Quick actions</p><p className="mt-1 text-[11px] text-foreground-muted">Each action is a native allowlisted deep link.</p></div><button type="button" disabled={disabled || actions.length >= 8} onClick={() => addArrayItem(sectionIndex, 'actions')} className="inline-flex items-center gap-1 rounded-lg border border-primary/30 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-primary-light disabled:opacity-50"><Plus size={12} aria-hidden="true" /> Add action</button></div>
  ...
  return (
    <section className="space-y-5 rounded-3xl border border-border bg-surface/[0.03] p-5 shadow-xl shadow-scrim" aria-labelledby="experience-editor-title">
      ...
      <section className="rounded-2xl border border-border bg-surface-subtle p-5" aria-labelledby="component-builder-title"><div className="flex items-center justify-between gap-3"><div><h3 id="component-builder-title" className="text-sm font-black uppercase tracking-wider text-foreground-muted">{homeLayoutMode ? 'Home layout sections' : 'Layout components'}</h3><p className="mt-1 text-xs text-foreground-muted">{homeLayoutMode ? 'Only the eight compiled Home Layout components can be added.' : 'Compose banners, campaign cards and experience sections from the registered schema.'}</p></div><button type="button" disabled={disabled} onClick={() => update({ sections: [...value.sections, newSection('hero_banner')] })} className="inline-flex items-center gap-1 rounded-xl border border-primary/30 bg-primary/10 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-primary-light disabled:opacity-50"><Plus size={14} aria-hidden="true" /> Add component</button></div>
