...
export default function AppExperienceEditor({
  ...
  const addArrayItem = (sectionIndex: number, key: 'items' | 'actions') => {
    ...
    const next = key === 'items'
      ? { id: createId('promo-item'), campaign_name: '', title: '', body: '', badge: '', alt_label: '', image_decorative: false, cta_label: 'Lihat sekarang', deep_link: '/food' }
  ...
  const componentOptions = homeLayoutMode ? HOME_LAYOUT_COMPONENTS : EXTENDED_COMPONENTS
  const availableComponentOptions = serviceVisibilityMode ? [['service_grid', 'Service visibility']] as const : componentOptions
  const enabledCount = value.sections.filter((section) => section.enabled !== false).length

  const renderTextField = (sectionIndex: number, key: string, label = key.replaceAll('_', ' '), placeholder = '') => key === 'alt_label' ? (
    <div key={key} className="space-y-2">
      <label className="text-xs font-bold capitalize text-foreground-muted">
        {label}
      ...
      <label className="flex items-center gap-2 text-[11px] font-semibold text-foreground-muted">
        <input type="checkbox" className="h-4 w-4 accent-primary" disabled={disabled} checked={value.sections[sectionIndex].properties.image_decorative === true} onChange={(event) => setProperty(sectionIndex, 'image_decorative', event.target.checked)} />
        Image is decorative (no alt text required)
      </label>
    </div>
  ) : (
    <label key={key} className="text-xs font-bold capitalize text-foreground-muted">
      {label}
      <input className={inputClass} disabled={disabled} value={String(value.sections[sectionIndex].properties[key] ?? '')} onChange={(event) => setProperty(sectionIndex, key, event.target.value)} placeholder={placeholder} />
    </label>
  )
  ...
  const renderCampaignIntro = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-info bg-info/[0.04] p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <label className="text-xs font-bold text-foreground-muted">Primary image / animation asset<select className={inputClass} disabled={disabled} value={mediaAssetId} onChange={(event) => updateCampaignIntroAssets(sectionIndex, event.target.value, '')}><option value="">No media — text-only intro</option>{mediaAssets.map((asset) => <option key={asset.asset_id} value={asset.asset_id}>{asset.asset_id} · {asset.kind}</option>)}</select></label>
        ...
        <label className="text-xs font-bold text-foreground-muted">Target display duration (seconds)<input type="number" min="1" max="60" className={inputClass} disabled={disabled} value={String(properties.display_duration_seconds ?? '')} onChange={(event) => setProperty(sectionIndex, 'display_duration_seconds', event.target.value === '' ? undefined : Number(event.target.value))} /></label>
        <label className="text-xs font-bold text-foreground-muted">Hard maximum duration (seconds)<input type="number" min="1" max="120" className={inputClass} disabled={disabled} value={String(properties.max_duration_seconds ?? '')} onChange={(event) => setProperty(sectionIndex, 'max_duration_seconds', event.target.value === '' ? undefined : Number(event.target.value))} /></label>
        <label className="text-xs font-bold text-foreground-muted">Frequency cap (hours)<input type="number" min="0" max="720" className={inputClass} disabled={disabled} value={String(properties.frequency_cap_hours ?? '')} onChange={(event) => setProperty(sectionIndex, 'frequency_cap_hours', event.target.value === '' ? undefined : Number(event.target.value))} /></label>
        <label className="text-xs font-bold text-foreground-muted">Maximum impressions<input type="number" min="1" max="100" className={inputClass} disabled={disabled} value={String(properties.max_impressions ?? '')} onChange={(event) => setProperty(sectionIndex, 'max_impressions', event.target.value === '' ? undefined : Number(event.target.value))} /></label>
        <label className="text-xs font-bold text-foreground-muted">Asset prefetch window (hours)<input type="number" min="1" max="24" className={inputClass} disabled={disabled} value={String(properties.prefetch_window_hours ?? '')} onChange={(event) => setProperty(sectionIndex, 'prefetch_window_hours', event.target.value === '' ? undefined : Number(event.target.value))} /></label>
        <label className="flex items-center gap-2 self-end rounded-xl border border-border bg-surface-subtle px-3 py-2.5 text-xs font-bold text-foreground-muted"><input type="checkbox" className="h-4 w-4 accent-primary" disabled={disabled} checked={properties.enabled !== false} onChange={(event) => setProperty(sectionIndex, 'enabled', event.target.checked)} /> Enabled</label>
      ...
      <div className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-3">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-foreground-muted">Headline pack key<input className={inputClass} disabled={disabled} value={String((properties.localized_copy as Record<string, unknown> | undefined)?.title ?? '')} onChange={(event) => setLocalizedReference(sectionIndex, 'title', event.target.value)} placeholder="campaign.intro.headline" /></label><label className="text-xs font-bold text-foreground-muted">Body pack key<input className={inputClass} disabled={disabled} value={String((properties.localized_copy as Record<string, unknown> | undefined)?.body ?? '')} onChange={(event) => setLocalizedReference(sectionIndex, 'body', event.target.value)} placeholder="campaign.intro.body" /></label></div>
      </div>
      <div className="mt-4 space-y-2 text-[11px] leading-relaxed text-foreground-muted">
        ...
        {displayDuration > maxDuration ? <p className="font-bold text-error">Display duration must not exceed the hard maximum; publish will be blocked until corrected.</p> : null}
      </div>
    </div>
  }
  ...
  const renderPromoItems = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-border bg-surface-subtle p-3">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-foreground-muted">Promo items</p><p className="mt-1 text-[11px] text-foreground-muted">Presentation only; campaign eligibility is resolved by Promo/Pricing.</p></div><button type="button" disabled={disabled || items.length >= 10} onClick={() => addArrayItem(sectionIndex, 'items')} className="inline-flex items-center gap-1 rounded-lg border border-primary/30 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-primary-light disabled:opacity-60"><Plus size={12} aria-hidden="true" /> Add item</button></div>
      <div className="mt-3 space-y-3">{items.map((item, itemIndex) => <div key={String(item.id ?? itemIndex)} className="rounded-xl border border-border p-3">
        <div className="mb-3 flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-widest text-foreground-muted">Item {itemIndex + 1}</span><button type="button" disabled={disabled || items.length <= 1} onClick={() => removeArrayItem(sectionIndex, 'items', itemIndex)} className="rounded-lg p-1 text-foreground-muted hover:bg-error-surface hover:text-error disabled:opacity-60" aria-label={`Remove promo item ${itemIndex + 1}`}><Trash2 size={14} aria-hidden="true" /></button></div>
        <div className="grid gap-3 md:grid-cols-2">
          {(['id', 'campaign_id', 'campaign_name', 'title', 'body', 'badge', 'alt_label', 'image_asset_id', 'cta_label', 'deep_link', 'external_url'] as const).map((key) => <label key={key} className="text-xs font-bold capitalize text-foreground-muted">{key === 'deep_link' ? 'Deep link (allowlisted)' : key === 'external_url' ? 'External URL (first-party)' : key.replaceAll('_', ' ')}<input className={inputClass} disabled={disabled} value={String(item[key] ?? '')} onChange={(event) => updateArrayItem(sectionIndex, 'items', itemIndex, { [key]: event.target.value })} placeholder={key === 'deep_link' ? '/food' : key === 'external_url' ? 'https://app.bawain.my.id/...' : ''} /></label>)}
          <label className="flex items-center gap-2 text-xs font-bold text-foreground-muted"><input type="checkbox" className="h-4 w-4 accent-primary" disabled={disabled} checked={item.image_decorative === true} onChange={(event) => updateArrayItem(sectionIndex, 'items', itemIndex, { image_decorative: event.target.checked })} /> Image is decorative (no alt text required)</label>
          <label className="text-xs font-bold text-foreground-muted">Localized title pack key<input className={inputClass} disabled={disabled} value={String((item.localized_copy as Record<string, unknown> | undefined)?.title ?? '')} onChange={(event) => setArrayItemLocalizedReference(sectionIndex, 'items', itemIndex, 'title', event.target.value)} placeholder="home.promo.title" /></label>
          <label className="text-xs font-bold text-foreground-muted">Localized subtitle/body pack key<input className={inputClass} disabled={disabled} value={String((item.localized_copy as Record<string, unknown> | undefined)?.body ?? '')} onChange={(event) => setArrayItemLocalizedReference(sectionIndex, 'items', itemIndex, 'body', event.target.value)} placeholder="home.promo.body" /></label>
        </div>
      </div>)}</div>
    </div>
  }
  ...
  const renderQuickActions = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-border bg-surface-subtle p-3">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-foreground-muted">Quick actions</p><p className="mt-1 text-[11px] text-foreground-muted">Each action is a native allowlisted deep link.</p></div><button type="button" disabled={disabled || actions.length >= 8} onClick={() => addArrayItem(sectionIndex, 'actions')} className="inline-flex items-center gap-1 rounded-lg border border-primary/30 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-primary-light disabled:opacity-60"><Plus size={12} aria-hidden="true" /> Add action</button></div>
      <div className="mt-3 space-y-3">{actions.map((action, itemIndex) => <div key={String(action.id ?? itemIndex)} className="rounded-xl border border-border p-3">
        <div className="mb-3 flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-widest text-foreground-muted">Action {itemIndex + 1}</span><button type="button" disabled={disabled || actions.length <= 1} onClick={() => removeArrayItem(sectionIndex, 'actions', itemIndex)} className="rounded-lg p-1 text-foreground-muted hover:bg-error-surface hover:text-error disabled:opacity-60" aria-label={`Remove quick action ${itemIndex + 1}`}><Trash2 size={14} aria-hidden="true" /></button></div>
        <div className="grid gap-3 md:grid-cols-2">{(['id', 'label', 'icon_asset_id', 'deep_link'] as const).map((key) => <label key={key} className="text-xs font-bold capitalize text-foreground-muted">{key.replaceAll('_', ' ')}<input className={inputClass} disabled={disabled} value={String(action[key] ?? '')} onChange={(event) => updateArrayItem(sectionIndex, 'actions', itemIndex, { [key]: event.target.value })} placeholder={key === 'deep_link' ? '/food' : ''} /></label>)}</div>
      </div>)}</div>
    </div>
  }

  return (
    <section className="space-y-5 rounded-3xl border border-border bg-surface/[0.03] p-5 shadow-xl shadow-scrim" aria-labelledby="experience-editor-title">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary-light">Approved component schema</p><h2 id="experience-editor-title" className="mt-1 text-xl font-black text-foreground-muted">{isRevision ? 'Create new revision' : homeLayoutMode ? 'Home layout editor' : 'Campaign / experience editor'}</h2><p className="mt-1 text-xs text-foreground-muted">Drag or use the arrow controls to change array order; the server validates the ordered schema before publish.</p></div><div className="flex gap-2"><button type="button" onClick={onCancel} className="rounded-xl border border-border px-3 py-2 text-xs font-black uppercase tracking-widest text-foreground-muted">Cancel</button><button type="button" disabled={disabled || saving || canaryError} onClick={() => onSave()} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-black uppercase tracking-widest text-on-primary disabled:opacity-60"><Save size={14} aria-hidden="true" />{saving ? 'Saving...' : isRevision ? 'Create revision' : 'Save draft'}</button></div></div>
      {broadWarning ? <div className="rounded-2xl border border-warning bg-warning-surface p-4 text-xs leading-relaxed text-warning">Broad public audience: server will require a different authorized operator to approve this campaign before publish.</div> : null}
      {canaryError ? <div className="rounded-2xl border border-error bg-error-surface p-4 text-xs text-error">Canary rollout requires an explicit internal/test cohort.</div> : null}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4"><label className="text-xs font-bold text-foreground-muted">Market code<input className={inputClass} disabled={disabled} value={value.market_code} onChange={(event) => update({ market_code: event.target.value })} placeholder="id-jk" /></label><label className="text-xs font-bold text-foreground-muted">Locale<input className={inputClass} disabled={disabled} value={value.locale} onChange={(event) => update({ locale: event.target.value })} placeholder="id-ID" /></label><label className="text-xs font-bold text-foreground-muted">Surface<select className={inputClass} disabled={disabled} value={value.surface} onChange={(event) => update({ surface: event.target.value as ExperienceForm['surface'] })}><option value="customer_android">Customer Android</option><option value="customer_web">Customer Web</option><option value="merchant_android">Merchant Android</option><option value="courier_android">Courier Android</option></select></label><label className="text-xs font-bold text-foreground-muted">Schema version<input type="number" min="1" max="10" className={inputClass} disabled={disabled} value={value.schema_version} onChange={(event) => update({ schema_version: Number(event.target.value) })} /></label><label className="text-xs font-bold text-foreground-muted">Minimum app version<input className={inputClass} disabled={disabled} value={value.min_app_version} onChange={(event) => update({ min_app_version: event.target.value })} placeholder="1.0.0" /></label><label className="text-xs font-bold text-foreground-muted">Maximum app version<input className={inputClass} disabled={disabled} value={value.max_app_version} onChange={(event) => update({ max_app_version: event.target.value })} placeholder="Optional" /></label><label className="text-xs font-bold text-foreground-muted">Start (local time)<input type="datetime-local" className={inputClass} disabled={disabled} value={value.starts_at} onChange={(event) => update({ starts_at: event.target.value })} /></label><label className="text-xs font-bold text-foreground-muted">End (local time)<input type="datetime-local" className={inputClass} disabled={disabled} value={value.ends_at} onChange={(event) => update({ ends_at: event.target.value })} /></label><label className="text-xs font-bold text-foreground-muted">Schedule timezone<input className={inputClass} disabled={disabled} value={value.schedule_timezone} onChange={(event) => update({ schedule_timezone: event.target.value })} placeholder="Asia/Jakarta" /></label><label className="text-xs font-bold text-foreground-muted">Rollout stage<select className={inputClass} disabled={disabled} value={value.rollout_stage} onChange={(event) => update({ rollout_stage: event.target.value as ExperienceForm['rollout_stage'], canary_cohort: event.target.value === 'canary' ? value.canary_cohort : '' })}><option value="public">Public</option><option value="canary">Canary / internal</option></select></label><label className="text-xs font-bold text-foreground-muted">Canary cohort<input className={inputClass} disabled={disabled || value.rollout_stage !== 'canary'} value={value.canary_cohort} onChange={(event) => update({ canary_cohort: event.target.value })} placeholder="internal-test" /></label><label className="text-xs font-bold text-foreground-muted">Percentage rollout<input type="number" min="0" max="100" className={inputClass} disabled={disabled} value={value.rollout_percentage} onChange={(event) => update({ rollout_percentage: Math.max(0, Math.min(100, Number(event.target.value))) })} /></label><label className="text-xs font-bold text-foreground-muted">Cache policy<select className={inputClass} disabled={disabled} value={value.cache_policy} onChange={(event) => update({ cache_policy: event.target.value as ExperienceForm['cache_policy'] })}><option value="private">Private</option><option value="no-store">No store</option><option value="public">Public</option></select></label></div>
      {value.rollout_percentage < 100 ? <p className="-mt-2 text-[11px] text-info">Percentage rollout uses a stable server-side bucket per authenticated user; users without an identity stay outside the partial rollout.</p> : null}
      <TargetingEditor value={value.targeting} disabled={disabled} onChange={(targeting) => update({ targeting })} />
      <section className="rounded-2xl border border-border bg-surface-subtle p-5" aria-labelledby="component-builder-title"><div className="flex items-center justify-between gap-3"><div><h3 id="component-builder-title" className="text-sm font-black uppercase tracking-wider text-foreground-muted">{homeLayoutMode ? 'Home layout sections' : 'Layout components'}</h3><p className="mt-1 text-xs text-foreground-muted">{homeLayoutMode ? 'Only the eight compiled Home Layout components can be added.' : 'Compose banners, campaign cards and experience sections from the registered schema.'}</p></div><button type="button" disabled={disabled} onClick={() => update({ sections: [...value.sections, newSection('hero_banner')] })} className="inline-flex items-center gap-1 rounded-xl border border-primary/30 bg-primary/10 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-primary-light disabled:opacity-60"><Plus size={14} aria-hidden="true" /> Add component</button></div>
        <div className="mt-4 space-y-4">{value.sections.map((section, index) => {
          const enabled = section.enabled !== false
          const options = availableComponentOptions.some(([key]) => key === section.component) ? availableComponentOptions : [...availableComponentOptions, [section.component, `${section.component} (existing)`] as const]
          return <article key={section.id} draggable={!disabled} onDragStart={() => setDragIndex(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragIndex !== null) moveSection(dragIndex, index); setDragIndex(null) }} onDragEnd={() => setDragIndex(null)} className={`rounded-2xl border p-4 ${enabled ? 'border-border bg-surface/[0.03]' : 'border-warning bg-warning/[0.04]'}`}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2 text-foreground-muted" title="Drag to reorder"><GripVertical size={17} aria-hidden="true" /><span className="text-[10px] font-black uppercase tracking-widest">Position {index + 1}</span></div><label className="flex flex-1 items-center gap-2 text-xs font-bold text-foreground-muted"><span className="sr-only">Enabled</span><input type="checkbox" disabled={disabled || (!enabled && enabledCount <= 1)} checked={enabled} onChange={(event) => setEnabled(index, event.target.checked)} className="h-4 w-4 accent-primary" />{enabled ? <Eye size={14} className="text-success" aria-hidden="true" /> : <EyeOff size={14} className="text-warning" aria-hidden="true" />} Enabled</label><div className="flex items-center gap-1"><button type="button" disabled={disabled || index === 0} onClick={() => moveSection(index, index - 1)} className="rounded-lg p-2 text-foreground-muted hover:bg-surface-subtle hover:text-foreground-muted disabled:opacity-60" aria-label={`Move section ${section.id} up`}><ArrowUp size={15} aria-hidden="true" /></button><button type="button" disabled={disabled || index === value.sections.length - 1} onClick={() => moveSection(index, index + 1)} className="rounded-lg p-2 text-foreground-muted hover:bg-surface-subtle hover:text-foreground-muted disabled:opacity-60" aria-label={`Move section ${section.id} down`}><ArrowDown size={15}  aria-hidden="true"/></button><button type="button" disabled={disabled || value.sections.length <= 1} aria-label={`Remove component ${section.id}`} onClick={() => removeSection(index)} className="rounded-lg p-2 text-foreground-muted transition hover:bg-error-surface hover:text-error disabled:opacity-60"><Trash2 size={16} aria-hidden="true" /></button></div></div>
