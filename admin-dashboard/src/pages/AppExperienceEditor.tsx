...
export default function AppExperienceEditor({
  ...
  const renderCampaignIntro = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-info bg-info/[0.04] p-4">
      ...
      <div className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-3">
        <p className="text-xs font-black uppercase tracking-wider text-primary-light">Localized headline/body pack keys</p>
      ...
      <div className="mt-4 space-y-2 text-xs leading-relaxed text-foreground-muted">
  ...
  const renderPromoItems = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-border bg-surface-subtle p-3">
	      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-foreground-muted">Promo items</p><p className="mt-1 text-xs text-foreground-muted">Presentation only; campaign eligibility is resolved by Promo/Pricing.</p></div><button type="button" disabled={disabled || items.length >= 10} onClick={() => addArrayItem(sectionIndex, 'items')} className="inline-flex items-center gap-1 rounded-lg border border-primary/30 px-2 py-1 text-xs font-black uppercase tracking-wide text-primary-light disabled:opacity-60"><Plus size={12} aria-hidden="true" /> Add item</button></div>
      <div className="mt-3 space-y-3">{items.map((item, itemIndex) => <div key={String(item.id ?? itemIndex)} className="rounded-xl border border-border p-3">
	        <div className="mb-3 flex items-center justify-between"><span className="text-xs font-black uppercase tracking-wide text-foreground-muted">Item {itemIndex + 1}</span><button type="button" disabled={disabled || items.length <= 1} onClick={() => removeArrayItem(sectionIndex, 'items', itemIndex)} className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-bold text-foreground-muted hover:bg-error-surface hover:text-error disabled:opacity-60" aria-label={`Remove promo item ${itemIndex + 1}`} title={`Remove promo item ${itemIndex + 1}`}><Trash2 size={14} aria-hidden="true" /><span>Remove</span></button></div>
  ...
  const renderQuickActions = (sectionIndex: number) => {
    ...
    return <div className="md:col-span-2 rounded-xl border border-border bg-surface-subtle p-3">
	      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-foreground-muted">Quick actions</p><p className="mt-1 text-xs text-foreground-muted">Each action is a native allowlisted deep link.</p></div><button type="button" disabled={disabled || actions.length >= 8} onClick={() => addArrayItem(sectionIndex, 'actions')} className="inline-flex items-center gap-1 rounded-lg border border-primary/30 px-2 py-1 text-xs font-black uppercase tracking-wide text-primary-light disabled:opacity-60"><Plus size={12} aria-hidden="true" /> Add action</button></div>
      <div className="mt-3 space-y-3">{actions.map((action, itemIndex) => <div key={String(action.id ?? itemIndex)} className="rounded-xl border border-border p-3">
	        <div className="mb-3 flex items-center justify-between"><span className="text-xs font-black uppercase tracking-wide text-foreground-muted">Action {itemIndex + 1}</span><button type="button" disabled={disabled || actions.length <= 1} onClick={() => removeArrayItem(sectionIndex, 'actions', itemIndex)} className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-bold text-foreground-muted hover:bg-error-surface hover:text-error disabled:opacity-60" aria-label={`Remove quick action ${itemIndex + 1}`} title={`Remove quick action ${itemIndex + 1}`}><Trash2 size={14} aria-hidden="true" /><span>Remove</span></button></div>
  ...
  return (
    <section className="space-y-5 rounded-3xl border border-border bg-surface/[0.03] p-5 shadow-xl shadow-scrim" aria-labelledby="experience-editor-title">
	      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-primary-light">Approved component schema</p><h2 id="experience-editor-title" className="mt-1 text-xl font-black text-foreground-muted">{isRevision ? 'Create new revision' : homeLayoutMode ? 'Home layout editor' : 'Campaign / experience editor'}</h2><p className="mt-1 text-xs text-foreground-muted">Drag or use the arrow controls to change array order; the server validates the ordered schema before publish.</p></div><div className="flex gap-2"><button type="button" onClick={onCancel} className="rounded-xl border border-border px-3 py-2 text-xs font-black uppercase tracking-wide text-foreground-muted">Cancel</button><button type="button" disabled={disabled || saving || canaryError} onClick={() => onSave()} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-black uppercase tracking-wide text-on-primary disabled:opacity-60"><Save size={14} aria-hidden="true" />{saving ? 'Saving...' : isRevision ? 'Create revision' : 'Save draft'}</button></div></div>
      ...
      {value.rollout_percentage < 100 ? <p className="-mt-2 text-xs text-info">Percentage rollout uses a stable server-side bucket per authenticated user; users without an identity stay outside the partial rollout.</p> : null}
      <TargetingEditor value={value.targeting} disabled={disabled} onChange={(targeting) => update({ targeting })} />
	      <section className="rounded-2xl border border-border bg-surface-subtle p-5" aria-labelledby="component-builder-title"><div className="flex items-center justify-between gap-3"><div><h3 id="component-builder-title" className="text-sm font-black uppercase tracking-wider text-foreground-muted">{homeLayoutMode ? 'Home layout sections' : 'Layout components'}</h3><p className="mt-1 text-xs text-foreground-muted">{homeLayoutMode ? 'Only the eight compiled Home Layout components can be added.' : 'Compose banners, campaign cards and experience sections from the registered schema.'}</p></div><button type="button" disabled={disabled} onClick={() => update({ sections: [...value.sections, newSection('hero_banner')] })} className="inline-flex items-center gap-1 rounded-xl border border-primary/30 bg-primary/10 px-3 py-2 text-xs font-black uppercase tracking-wide text-primary-light disabled:opacity-60"><Plus size={14} aria-hidden="true" /> Add component</button></div>
