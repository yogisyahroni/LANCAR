  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><button type="button" onClick={onRefresh} className="inline-flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-[10px] font-black uppercase tracking-widest text-foreground-muted"><RefreshCw size={14} aria-hidden="true" /> Refresh</button><button type="button" disabled={!canEdit} onClick={onNew} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-[10px] font-black uppercase tracking-widest text-on-primary disabled:opacity-50"><FilePlus2 size={14} aria-hidden="true" /> New token draft</button></div>
      </div>
      <div className="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-6">
          ...
          <section className="rounded-3xl border border-border bg-surface/[0.03] p-5" aria-labelledby="design-token-lifecycle-title"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="design-token-lifecycle-title" className="text-base font-black text-foreground-muted">Draft and release lifecycle</h2><p className="mt-1 text-xs leading-relaxed text-foreground-muted">Saving creates or updates a manifest draft. Publish is disabled until the accessibility gate passes and the normal permission/approval checks are satisfied.</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!canEdit || saving} onClick={() => onSave(saveForm)} className="rounded-xl bg-primary px-3 py-2 text-[10px] font-black uppercase tracking-widest text-on-primary disabled:opacity-50">{saving ? 'Saving…' : 'Save draft'}</button><button type="button" disabled={approvalDisabled} onClick={onApprove} className="inline-flex items-center gap-2 rounded-xl border border-success bg-success-surface px-3 py-2 text-[10px] font-black uppercase tracking-widest text-success disabled:opacity-50"><ShieldCheck size={14} aria-hidden="true" /> Approve</button><button type="button" disabled={publishDisabled} onClick={onPublish} className="inline-flex items-center gap-2 rounded-xl bg-success px-3 py-2 text-[10px] font-black uppercase tracking-widest text-on-success disabled:opacity-50"><Send size={14} aria-hidden="true" /> Publish</button></div></div>{!validation.pass ? <p role="alert" className="mt-4 rounded-xl border border-error bg-error/[0.06] p-3 text-xs text-error">Contrast validation failed. Correct the bounded presets before publishing.</p> : <p className="mt-4 rounded-xl border border-success bg-success/[0.06] p-3 text-xs text-success">All light/dark palette checks pass WCAG AA.</p>}</section>
