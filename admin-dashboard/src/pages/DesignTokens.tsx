  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="text-xs font-black uppercase tracking-wide text-primary-light">Safe runtime configuration</p><h1 className="mt-2 text-3xl font-black tracking-tight text-foreground-muted">Design Tokens · App Experience</h1><p className="mt-2 max-w-3xl text-sm leading-relaxed text-foreground-muted">Edit only campaign presentation presets. The existing manifest draft, maker-checker approval, publish, audit and rollback lifecycle remains the source of truth.</p></div>
        <div className="flex flex-wrap gap-2"><button type="button" onClick={onRefresh} className="inline-flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-xs font-black uppercase tracking-wide text-foreground-muted"><RefreshCw size={14} aria-hidden="true" /> Refresh</button><button type="button" disabled={!canEdit} onClick={onNew} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-black uppercase tracking-wide text-on-primary disabled:opacity-60"><FilePlus2 size={14} aria-hidden="true" /> New token draft</button></div>
      </div>
      <div className="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
        <aside className="space-y-4 rounded-3xl border border-border bg-surface/[0.03] p-4" aria-labelledby="design-token-manifest-list">
          ...
          <div className="space-y-2">{manifests.map((manifest) => <button type="button" key={manifest.manifest_id} onClick={() => onSelect(manifest)} className={`w-full rounded-2xl border p-3 text-left ${manifest.manifest_id === selectedId ? 'border-primary/50 bg-primary/10' : 'border-border bg-surface-subtle hover:border-border'}`}><div className="flex items-center justify-between gap-2"><span className="truncate font-mono text-xs text-foreground-muted" title={manifest.manifest_id}>{manifest.manifest_id.slice(0, 8)}…</span><StatusBadge status={manifest.state} labelPrefix="Manifest status" className="px-2 py-1" /></div><p className="mt-2 text-sm font-black text-foreground-muted">r{manifest.revision} · {manifest.surface}</p><p className="mt-1 text-xs text-foreground-muted">{manifest.market_code} · {manifest.locale}</p></button>)}</div>
        ...
        <div className="min-w-0 space-y-6">
          <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-surface-subtle p-4 text-xs text-foreground-muted"><span className="font-bold text-foreground-muted">{selected ? `Manifest ${selected.manifest_id.slice(0, 8)}…` : 'New manifest'}</span><span>·</span><span>Mode: {mode}</span>{actionManifest ? <><StatusBadge status={actionManifest.state} labelPrefix="Token manifest state" />{actionManifest.requires_approval ? <StatusBadge status={actionManifest.approval_status} labelPrefix="Token approval status" /> : <StatusBadge status="ready" label="Persetujuan tidak diperlukan" labelPrefix="Token approval status" />}</> : <StatusBadge status="ready" label="Belum disimpan" labelPrefix="Token manifest state" className="border-border bg-surface-subtle text-foreground-muted" />}</div>
          ...
          <section className="rounded-3xl border border-border bg-surface/[0.03] p-5" aria-labelledby="design-token-lifecycle-title"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="design-token-lifecycle-title" className="text-base font-black text-foreground-muted">Draft and release lifecycle</h2><p className="mt-1 text-xs leading-relaxed text-foreground-muted">Saving creates or updates a manifest draft. Publish is disabled until the accessibility gate passes and the normal permission/approval checks are satisfied.</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!canEdit || saving} onClick={() => onSave(saveForm)} className="rounded-xl bg-primary px-3 py-2 text-xs font-black uppercase tracking-wide text-on-primary disabled:opacity-60">{saving ? 'Saving…' : 'Save draft'}</button><button type="button" disabled={approvalDisabled} onClick={onApprove} className="inline-flex items-center gap-2 rounded-xl border border-success bg-success-surface px-3 py-2 text-xs font-black uppercase tracking-wide text-success disabled:opacity-60"><ShieldCheck size={14} aria-hidden="true" /> Approve</button><button type="button" disabled={publishDisabled} onClick={onPublish} className="inline-flex items-center gap-2 rounded-xl bg-success px-3 py-2 text-xs font-black uppercase tracking-wide text-on-success disabled:opacity-60"><Send size={14} aria-hidden="true" /> Publish</button></div></div>{!validation.pass ? <p role="alert" className="mt-4 rounded-xl border border-error bg-error/[0.06] p-3 text-xs text-error">Contrast validation failed. Correct the bounded presets before publishing.</p> : <p className="mt-4 rounded-xl border border-success bg-success/[0.06] p-3 text-xs text-success">All light/dark palette checks pass WCAG AA.</p>}</section>
          ...
          {publishedManifest ? <p className="text-xs text-foreground-muted">Published revision r{publishedManifest.revision} remains the live source until an approved candidate is published or rolled back.</p> : null}
        </div>
      </div>
    </div>
  )
