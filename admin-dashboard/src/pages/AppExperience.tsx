...
import { Ban, Check, Eye, FilePlus2, Layers3, RefreshCw, Send, ShieldCheck, Sparkles } from 'lucide-react'
...
export default function AppExperience() {
  ...
  useEffect(() => {
    ...
    setPreviewSurface(selected.surface)
  ...
  const startNew = () => {
    const next = defaultExperienceForm()
    setSelectedId(null)
    setForm(next)
    setPreviewSurface(next.surface)
    setPreviewAudience({ market_code: next.market_code, locale: next.locale, app_version: next.min_app_version, cohort: '' })
    setMode('new')
    setSimulation(undefined)
  }
  const selectManifest = (manifest: ExperienceManifest) => setSelectedId(manifest.manifest_id)
  const save = () => saveMutation.mutate()
  const actionBusy = previewMutation.isPending || approveMutation.isPending || publishMutation.isPending || rollbackMutation.isPending || killMutation.isPending || restoreMutation.isPending
