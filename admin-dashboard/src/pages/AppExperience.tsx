...
import ExperienceScopeBar, {
  type ExperienceScope,
} from "../components/experience/ExperienceScopeBar";
...
import {
  ...
  defaultServiceVisibilityForm,
  detectExperienceConflicts,
  experienceCampaignStatus,
  formFromManifest,
  formToPayload,
  hasAudienceConstraints,
  type ExperienceForm,
...
export default function AppExperience() {
  ...
  const canEdit = hasExperiencePermission(
    user,
    EXPERIENCE_CAPABILITIES.draftWrite,
  );
  const canSubmitApproval = hasExperiencePermission(
    user,
    EXPERIENCE_CAPABILITIES.submitApproval,
  );
  const canApprove = hasExperiencePermission(
    user,
    EXPERIENCE_CAPABILITIES.approve,
  );
  const canPublish = hasExperiencePermission(
    user,
    EXPERIENCE_CAPABILITIES.publish,
  );
  const canKill = hasExperiencePermission(
    user,
    EXPERIENCE_CAPABILITIES.killSwitchExecute,
  );
  ...
  const [selectedId, setSelectedId] = useState<string | null>(() =>
    searchParams.get("manifest_id"),
  );
  const revisionQuery = Number(searchParams.get("revision"));
  const selectedRevision =
    Number.isInteger(revisionQuery) && revisionQuery > 0 ? revisionQuery : null;
  const focusAssetId = searchParams.get("asset_id");
  const [form, setForm] = useState<ExperienceForm>(() =>
    location.pathname.endsWith("/service-visibility")
      ? defaultServiceVisibilityForm()
      : defaultExperienceForm(),
  );
  const [mode, setMode] = useState<"new" | "draft" | "revision">("new");
  const [previewSurface, setPreviewSurface] =
    useState<ExperienceSurface>("customer_android");
  const [previewAudience, setPreviewAudience] = useState<PreviewAudience>({
    market_code: "id-jk",
    locale: "id-ID",
    app_version: "1.0.0",
    schema_version: 1,
  ...
  const [simulation, setSimulation] = useState<{
    matched?: boolean;
    reason?: string;
    selected_manifest?: { revision?: number } | null;
  }>();
  const [previewResult, setPreviewResult] = useState<PreviewResult>();
  const [audiencePair, setAudiencePair] = useState<{
    matching?: typeof simulation;
    nonMatching?: typeof simulation;
  }>({});
  const activeSection =
    appExperienceNavigationItem(location.pathname)?.label ?? "Overview";
  const audienceWorkspace =
    activeSection === "Audience & Targeting" || activeSection === "Scheduling";
  ...
  const manifestsQuery = useQuery({
    ...
    queryFn: async (): Promise<ExperienceManifest[]> =>
      (
        await api.get("/admin/experience/manifests", {
          params: { market_code: form.market_code, surface: form.surface },
        })
      ).data?.data ?? [],
  });
  const selected = useMemo(
    () =>
      manifestsQuery.data?.find(
        (manifest) => manifest.manifest_id === selectedId,
      ) ?? null,
    [manifestsQuery.data, selectedId],
  );
  const historyQuery = useQuery({
    queryKey: ["experience-manifest-history", selectedId],
    enabled: Boolean(selectedId) && activeSection !== "Overview",
    queryFn: async (): Promise<ExperienceHistory> =>
      (await api.get(`/admin/experience/manifests/${selectedId}`)).data
        ?.data ?? { revisions: [], audit: [] },
  });
  const draft =
    historyQuery.data?.revisions.find(
      (revision) => revision.state === "draft",
    ) ?? (selected?.state === "draft" ? selected : null);
  const actionManifest = draft ?? selected;
  const publishedManifest =
    historyQuery.data?.revisions.find(
      (revision) => revision.state === "published",
    ) ?? (selected?.state === "published" ? selected : null);
  ...
  useEffect(() => {
    ...
    setPreviewAudience({
      market_code: editorManifest.market_code,
      locale: editorManifest.locale,
      app_version: editorManifest.min_app_version,
      schema_version: editorManifest.schema_version,
      device_preset: "phone",
      theme_mode: "system",
      city_code: "",
      zone_code: "",
  ...
  const saveMutation = useMutation({
    mutationFn: async (formOverride?: ExperienceForm) => {
      ...
      if (mode === "draft" && nextForm.manifest_id) {
        if (!actionManifest?.checksum || actionManifest.state !== "draft")
          throw new Error("Refresh the draft before saving it");
        return api.patch(
          `/admin/experience/manifests/${nextForm.manifest_id}/draft`,
          payload,
          {
            headers: {
      ...
      return api.post("/admin/experience/manifests", payload, {
    ...
    onSuccess: (response) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["experience-manifest-history", manifest.manifest_id],
      });
  ...
  const previewMutation = useMutation({
    mutationFn: async () => {
      if (!actionManifest || actionManifest.state !== "draft")
        throw new Error("Save a draft before previewing it");
      const response = await api.post(
        `/admin/experience/manifests/${actionManifest.manifest_id}/preview`,
        {
          audience: {
            ...previewAudience,
            app_version: String(previewAudience.app_version),
            user_id: previewAudience.user_id.trim() || null,
            cohort: previewAudience.cohort || null,
          },
        },
      );
    ...
    onSuccess: (response) => {
      ...
      setPreviewResult(response.preview_result as PreviewResult);
      toast.success("Server preview refreshed");
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const previewAudienceFor = (nonMatching: boolean) => {
    ...
    return {
      ...previewAudience,
      market_code: nonMatching
        ? "zz-zz"
        : targeting.market_codes[0] || form.market_code,
      locale: targeting.locales[0] || form.locale,
      app_version: form.min_app_version,
      city_code: nonMatching
        ? targeting.city_codes[0]
          ? `${targeting.city_codes[0]}-other`
          : "non-target-city"
        : targeting.city_codes[0] || null,
      zone_code: nonMatching
        ? targeting.zone_codes[0]
          ? `${targeting.zone_codes[0]}-other`
          : "non-target-zone"
        : targeting.zone_codes[0] || null,
  ...
  const audiencePairMutation = useMutation({
    mutationFn: async (nonMatching: boolean) => {
      if (!actionManifest || actionManifest.state !== "draft")
        throw new Error("Save a draft before simulating an audience");
      const response = await api.post(
        `/admin/experience/manifests/${actionManifest.manifest_id}/preview`,
        { audience: previewAudienceFor(nonMatching) },
      );
      return {
        nonMatching,
        simulation: response.data?.simulation as typeof simulation,
      };
    },
    onSuccess: ({ nonMatching, simulation: nextSimulation }) => {
      setAudiencePair((previous) => ({
        ...previous,
        ...(nonMatching
          ? { nonMatching: nextSimulation }
          : { matching: nextSimulation }),
      }));
      toast.success(
        nonMatching
          ? "Non-matching audience simulated"
          : "Matching audience simulated",
      );
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const approveMutation = useMutation({
    mutationFn: () =>
      api.post(
        `/admin/experience/manifests/${actionManifest?.manifest_id}/approve`,
        {},
        { headers: { "X-Idempotency-Key": requestKey("approve") } },
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["experience-manifests"] });
      queryClient.invalidateQueries({
      ...
      toast.success("Campaign approved by checker");
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const submitApprovalMutation = useMutation({
    mutationFn: () =>
      api.post(
        `/admin/experience/manifests/${actionManifest?.manifest_id}/submit-approval`,
        {},
        { headers: { "X-Idempotency-Key": requestKey("submit_approval") } },
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["experience-manifests"] });
      queryClient.invalidateQueries({
      ...
      toast.success("Approval candidate frozen and submitted");
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const publishMutation = useMutation({
    mutationFn: () =>
      api.post(
        `/admin/experience/manifests/${actionManifest?.manifest_id}/publish`,
        {},
        { headers: { "X-Idempotency-Key": requestKey("publish") } },
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["experience-manifests"] });
      queryClient.invalidateQueries({
  ...
  const rollbackMutation = useMutation({
    mutationFn: (revision: number) =>
      api.post(
        `/admin/experience/manifests/${actionManifest?.manifest_id}/rollback`,
        {
          target_revision: revision,
          reason: "Known-good revision restored from App Experience CMS",
        },
  ...
  const killMutation = useMutation({
    mutationFn: () =>
      api.post(
        `/admin/experience/manifests/${actionManifest?.manifest_id}/kill`,
        { reason: "Emergency disable from App Experience CMS" },
        { headers: { "X-Idempotency-Key": requestKey("kill") } },
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["experience-manifests"] });
      queryClient.invalidateQueries({
      ...
      toast.success("Campaign kill switch enabled");
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const restoreMutation = useMutation({
    mutationFn: () =>
      api.post(
        `/admin/experience/manifests/${actionManifest?.manifest_id}/restore`,
        { reason: "Approved restoration from App Experience CMS" },
        { headers: { "X-Idempotency-Key": requestKey("restore") } },
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["experience-manifests"] });
      queryClient.invalidateQueries({
      ...
      toast.success("Campaign kill switch cleared");
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const retireMutation = useMutation({
    mutationFn: () =>
      api.post(
        `/admin/experience/manifests/${publishedManifest?.manifest_id}/retire`,
        { reason: "Campaign retired from App Experience CMS" },
        { headers: { "X-Idempotency-Key": requestKey("retire") } },
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["experience-manifests"] });
      queryClient.invalidateQueries({
      ...
      toast.success("Campaign retired");
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const startNew = () => {
    ...
    setPreviewAudience({
      market_code: next.market_code,
      locale: next.locale,
      app_version: next.min_app_version,
  ...
  const selectManifest = (manifest: ExperienceManifest) =>
    setSelectedId(manifest.manifest_id);
  const duplicate = () => {
    const source = editorManifest ?? selected;
    if (!source) return;
    const { manifest_id: _manifestId, ...duplicateForm } =
      formFromManifest(source);
    setSelectedId(null);
    setForm(duplicateForm);
    setPreviewSurface(duplicateForm.surface);
    setPreviewAudience({
      market_code: duplicateForm.market_code,
      locale: duplicateForm.locale,
      app_version: duplicateForm.min_app_version,
      schema_version: duplicateForm.schema_version,
      device_preset: "phone",
      theme_mode: "system",
      city_code: "",
      zone_code: "",
  ...
  if (activeSection === "Asset Library")
    return (
  ...
  if (activeSection === "Design Tokens")
    return (
      <div className="space-y-8 animate-in">
        <ExperienceScopeBar value={scope} onChange={updateScope} />
        <DesignTokens
          ...
          publishedManifest={publishedManifest}
          ...
          onRefresh={() => {
            ...
            if (selectedId) void historyQuery.refetch();
          }}
        ...
        >
          ...
          <div className="space-y-2">
            {(manifestsQuery.data ?? []).map((manifest) => {
              const status = experienceCampaignStatus(manifest);
              return (
                <button
                  type="button"
                  key={manifest.id}
        ...
        <div className="min-w-0 space-y-6">
          {mode === "new" || form.manifest_id ? (
            <AppExperienceEditor
              ...
              onCancel={() => {
                if (selected) {
                  const restored = formFromManifest(selected);
                  setForm(restored);
                  setPreviewSurface(restored.surface);
          ...
          {audienceWorkspace ? (
            <TargetingSummary
              form={form}
              conflicts={detectExperienceConflicts(
                form,
                manifestsQuery.data ?? [],
                selectedId,
              )}
              scheduleOnly={activeSection === "Scheduling"}
            />
          ) : null}
          <section
          ...
          >
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {actionManifest?.requires_approval &&
                actionManifest.approval_status === "pending" ? (
                  <button
                    type="button"
                ...
                {publishedManifest && canPublish ? (
                  <button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Retire this campaign? It will stop public exposure and remain in revision history.",
                        )
                      )
              ...
              <label className="text-xs font-bold text-foreground-muted">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setPreviewAudience({
                      ...previewAudience,
                      market_code: event.target.value,
                    })
                  }
                />
              </label>
              ...
              <label className="text-xs font-bold text-foreground-muted">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setPreviewAudience({
                      ...previewAudience,
                      schema_version: Number(event.target.value) || 1,
                    })
                  }
                />
              </label>
              <label className="text-xs font-bold text-foreground-muted">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setPreviewAudience({
                      ...previewAudience,
                      device_preset: event.target
                        .value as PreviewAudience["device_preset"],
                    })
                  }
                >
              ...
              <label className="text-xs font-bold text-foreground-muted">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setPreviewAudience({
                      ...previewAudience,
                      theme_mode: event.target
                        .value as PreviewAudience["theme_mode"],
                    })
                  }
                >
              ...
              <label className="text-xs font-bold text-foreground-muted">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setPreviewAudience({
                      ...previewAudience,
                      user_status: event.target
                        .value as PreviewAudience["user_status"],
                    })
                  }
                >
          ...
          {audienceWorkspace ? (
            ...
            >
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {(
                  [
                    ["matching", "Matching case", audiencePair.matching],
                    [
                      "nonMatching",
                      "Non-matching case",
                      audiencePair.nonMatching,
                    ],
                  ] as const
                  ...
                  >
                    ...
                    >
                      {result
                        ? result.matched
                          ? "matched"
                          : `not matched · ${result.reason || "unknown"}`
                        : "Not run"}
                    </p>
          ...
          <ExperiencePreview
            ...
            resolvedSections={
              previewResult
                ? (previewResult.candidate?.manifest.sections ??
                  previewResult.fallback?.sections ??
                  [])
                : undefined
            }
          ...
          {selectedId && historyQuery.data ? (
            <RevisionHistory
              ...
              audit={historyQuery.data.audit}
              canRollback={
                canRollback &&
                historyQuery.data.revisions.some(
                  (revision) => revision.state === "published",
                )
              }
