    ...
    <div className="space-y-8 animate-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
        ...
        <div className="flex gap-3">
          <button type="button" onClick={() => experimentsQuery.refetch()} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-xs font-black uppercase tracking-wide text-foreground-muted"><RefreshCw size={14} aria-hidden="true" /> Refresh</button>
          <button type="button" onClick={() => setShowCreate(true)} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-xs font-black uppercase tracking-wide text-on-primary"><Plus size={14} aria-hidden="true" /> New experiment</button>
        </div>
      </div>
        ...
        {(experimentsQuery.data ?? []).map((experiment) => (
          <article key={experiment.id} className="glass-card rounded-3xl border-border p-6 shadow-xl shadow-scrim">
            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-5">
              <div>
                <div className="flex items-center gap-3 flex-wrap"><h2 className="text-lg font-black text-foreground-muted">{experiment.name}</h2><StatusBadge status={experiment.status} labelPrefix="Experiment status" /></div>
