  ...
  return (
    <div className="space-y-8 animate-in">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="glass-card rounded-2xl border-border p-5"><p className="text-xs font-bold uppercase tracking-widest text-foreground-muted">Visible cases</p><p className="mt-2 text-3xl font-black text-foreground-muted">{casesQuery.data?.total ?? '—'}</p></div>
        <div className="glass-card rounded-2xl border-warning p-5"><p className="text-xs font-bold uppercase tracking-widest text-warning">Open workload</p><p className="mt-2 text-3xl font-black text-warning">{openCount}</p></div>
        <div className="glass-card rounded-2xl border-error p-5"><p className="text-xs font-bold uppercase tracking-widest text-error">SLA breached</p><p className="mt-2 text-3xl font-black text-error">{breachedCount}</p></div>
      </div>
        ...
        {casesQuery.isLoading ? <div className="p-12 text-center text-sm text-foreground-muted">Loading support cases…</div> : casesQuery.isError ? <div className="p-12 text-center text-sm text-error">Support case queue unavailable.</div> : data.length === 0 ? <div className="p-12 text-center"><CheckCircle2 className="mx-auto text-success" aria-hidden="true" /><p className="mt-3 font-semibold text-foreground-muted">No cases for this filter</p></div> : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[960px] text-left text-sm">
              <thead className="border-b border-border bg-surface/[0.03] text-[10px] uppercase tracking-widest text-foreground-muted"><tr><th scope="col" className="px-6 py-4">Case</th><th scope="col" className="px-6 py-4">Service / market</th><th scope="col" className="px-6 py-4">SLA</th><th scope="col" className="px-6 py-4">Owner</th><th scope="col" className="px-6 py-4" /></tr></thead>
              <tbody className="divide-y divide-border">
                {data.map((item) => <tr key={item.id} className="transition hover:bg-surface/[0.03]">
                  <td className="px-6 py-5"><div className="flex items-start gap-3"><StatusBadge status={item.status} labelPrefix="Case status" className="mt-0.5 text-[10px] uppercase tracking-wider" /><div><p className="font-bold text-foreground-muted">{item.case_number} · {item.subject}</p><p className="mt-1 text-xs text-foreground-muted">{item.category} · {item.priority}</p></div></div></td>
      ...
      {detail && <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-8">
        ...
        <div className="glass-card relative z-10 max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[32px] border-border p-6 md:p-10">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-primary-light">{detail.case_number}</p><h2 className="mt-2 text-3xl font-black tracking-tight text-foreground-muted">{detail.subject}</h2><p className="mt-2 text-sm text-foreground-muted">{detail.category} · {detail.service_code} · {detail.market_code}</p></div><button type="button" onClick={() => setSelected(null)} className="rounded-xl border border-border p-2 text-foreground-muted hover:text-foreground"><X size={20} aria-hidden="true" /></button></div>
