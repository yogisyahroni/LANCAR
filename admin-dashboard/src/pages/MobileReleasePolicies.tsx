  ...
  return (
    <div className="space-y-6 p-6 lg:p-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex gap-2"><button type="button" onClick={() => query.refetch()} className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-4 py-3 text-xs font-black uppercase tracking-widest text-foreground-muted"><RefreshCw size={14} aria-hidden="true" /> Refresh</button>{canMutate ? <button type="button" onClick={() => { setForm(newForm()); setShowForm(true) }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-xs font-black uppercase tracking-widest text-on-primary"><Save size={14} aria-hidden="true" /> New policy</button> : null}</div>
      </header>

      <section className="rounded-3xl border border-warning bg-warning-surface p-5" aria-label="release policy safety boundary">
        <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 shrink-0 text-warning" size={20} aria-hidden="true" /><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-black text-warning">Release metadata only</p><span className="rounded-full border border-error bg-error-surface px-2 py-1 text-[10px] font-black uppercase tracking-widest text-error">Requires App Release</span></div><p className="mt-1 text-xs leading-relaxed text-warning">Remote policy tidak mengirim kode native, JavaScript executable, atau menggantikan store review. Capability native baru diberi label <strong>Requires App Release</strong> dan tetap memerlukan build serta review platform.</p></div></div>
      </section>
      ...
      {showForm ? <section className="rounded-3xl border border-border bg-surface/[0.03] p-5 shadow-xl shadow-scrim" aria-labelledby="release-policy-editor-title">
        <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary-light">Scoped policy editor</p><h2 id="release-policy-editor-title" className="mt-1 text-xl font-black text-foreground-muted">Set release gate</h2></div><CheckCircle2 className="text-success" size={20}  aria-hidden="true"/></div>
        ...
        {isHard ? <section className="mt-5 rounded-2xl border border-info bg-info-surface p-4" aria-label="hard update impact estimate"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-info">Affected version estimate</p><p className="mt-1 text-[11px] text-info">Observed experience telemetry, last {impactQuery.data?.window_days ?? 30} days. This is a release-safety sample, not a fabricated total-user count.</p></div><span className="rounded-full bg-surface-subtle px-2 py-1 text-[10px] font-black uppercase tracking-widest text-info">{impactQuery.isLoading ? 'Loading' : impactQuery.data?.coverage === 'observed' ? `${impactQuery.data.observed_events} events` : 'No sample'}</span></div>{impactQuery.isError ? <p className="mt-3 text-xs text-error">Impact estimate unavailable; hard update remains blocked.</p> : null}{impactQuery.data?.coverage === 'no_observed_events' ? <p className="mt-3 text-xs text-warning">Belum ada telemetry versi untuk scope ini. Hard update tidak dapat dipublish sampai sample teramati tersedia.</p> : null}{impactQuery.data?.coverage === 'observed' ? <div className="mt-3 grid gap-2 sm:grid-cols-3"><div className="rounded-xl bg-surface-subtle p-3"><p className="text-[10px] uppercase tracking-widest text-foreground-muted">Affected</p><p className="mt-1 text-lg font-black text-error">{impactQuery.data.affected_share_pct == null ? '—' : `${impactQuery.data.affected_share_pct}%`}</p></div><div className="rounded-xl bg-surface-subtle p-3"><p className="text-[10px] uppercase tracking-widest text-foreground-muted">Unknown</p><p className="mt-1 text-lg font-black text-warning">{impactQuery.data.unknown_events}</p></div><div className="rounded-xl bg-surface-subtle p-3"><p className="text-[10px] uppercase tracking-widest text-foreground-muted">Recovery</p><p className="mt-1 text-xs font-black text-success">Order + support open</p></div></div> : null}{impactQuery.data?.versions.length ? <div className="mt-3 space-y-1">{impactQuery.data.versions.map((version) => <div key={version.app_version} className="flex items-center justify-between rounded-lg bg-surface-subtle px-3 py-2 text-[11px]"><span className="font-mono text-foreground-muted">{version.app_version}</span><span className="text-foreground-muted">{version.share_pct}% · {version.affected === null ? 'unknown' : version.affected ? 'affected' : 'supported'}</span></div>)}</div> : null}</section> : null}
        <div className="mt-5 flex flex-wrap items-center gap-3"><button type="button" disabled={!canSave || !canMutate || saveMutation.isPending} onClick={save} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-xs font-black uppercase tracking-widest text-on-primary disabled:opacity-50"><Save size={14} aria-hidden="true" /> {saveMutation.isPending ? 'Saving...' : isHard ? 'Confirm and publish policy' : 'Save policy'}</button><button type="button" onClick={() => setShowForm(false)} className="rounded-xl border border-border px-4 py-3 text-xs font-black uppercase tracking-widest text-foreground-muted">Cancel</button><p className="text-xs text-foreground-muted">Perubahan hard dilindungi permission policy, TOTP, konfirmasi eksplisit, reason audit dan idempotency key.</p></div>
      </section> : null}

      <section className="rounded-3xl border border-border bg-surface/[0.03] p-5" aria-labelledby="release-policy-list-title">
        <div className="flex items-center justify-between gap-3"><div><h2 id="release-policy-list-title" className="text-lg font-black text-foreground-muted">Active policy matrix</h2><p className="mt-1 text-xs text-foreground-muted">One policy per market, client, and platform. Revision history is retained server-side.</p></div><span className="rounded-full bg-surface-subtle px-3 py-1 text-[10px] font-black uppercase tracking-widest text-foreground-muted">{sorted.length} policies</span></div>
