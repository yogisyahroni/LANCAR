       ...
       <div className="fixed inset-0 z-50 flex items-center justify-center bg-scrim/70 p-4">
         <FocusTrap className="w-full max-w-lg">
           <form role="dialog" aria-modal="true" aria-labelledby="meeting-point-dialog-title" onSubmit={(event) => { event.preventDefault(); if (!draft.name.trim() || !draft.address.trim() || !Number.isFinite(Number(draft.latitude)) || !Number.isFinite(Number(draft.longitude))) { toast.error('Nama, alamat, dan koordinat wajib valid'); return } save.mutate(draft) }} className="w-full space-y-4 rounded-3xl border border-border bg-background p-6">
             ...
             <label className="block text-sm text-foreground-muted">Nama (wajib)<input value={draft.name} aria-required="true" onChange={(e) => setDraft({ ...draft, name: e.target.value })} className="mt-1 w-full rounded-xl border border-border bg-surface-subtle p-3 text-foreground" required /></label>
             <label className="block text-sm text-foreground-muted">Alamat (wajib)<input value={draft.address} aria-required="true" onChange={(e) => setDraft({ ...draft, address: e.target.value })} className="mt-1 w-full rounded-xl border border-border bg-surface-subtle p-3 text-foreground" required /></label>
             <div className="grid grid-cols-3 gap-3">
               ...
               <label className="text-sm text-foreground-muted">Latitude (wajib)<input type="number" step="any" value={draft.latitude} aria-required="true" onChange={(e) => setDraft({ ...draft, latitude: Number(e.target.value) })} className="mt-1 w-full rounded-xl border border-border bg-surface-subtle p-3 text-foreground" required /></label>
               <label className="text-sm text-foreground-muted">Longitude (wajib)<input type="number" step="any" value={draft.longitude} aria-required="true" onChange={(e) => setDraft({ ...draft, longitude: Number(e.target.value) })} className="mt-1 w-full rounded-xl border border-border bg-surface-subtle p-3 text-foreground" required /></label>
             </div>
