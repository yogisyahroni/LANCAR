    ...
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-border bg-surface-subtle p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {detail?.config && <div className={`rounded-full px-3 py-1 text-xs font-bold ${detail.readiness?.is_ready ? 'bg-success-surface text-success' : 'bg-warning-surface text-warning'}`}>{readinessText}</div>}
        </div>
      </section>
      ...
      {detailQuery.isLoading || !draft || !detail ? <AdminPageSkeleton /> : <>
        <section className="rounded-3xl border border-border bg-surface-subtle p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            {canWrite && <button disabled={updateMutation.isPending} onClick={() => updateMutation.mutate()} className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-black text-on-primary disabled:opacity-50"><Save className="h-4 w-4" aria-hidden="true" /> Simpan config</button>}
          </div>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {(['country_code', 'region_code', 'currency_code', 'default_locale', 'timezone'] as const).map((key) => <label className={labelClass} key={key}><span className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">{key}</span><input className={inputClass} value={draft[key]} onChange={(event) => updateDraft(key, event.target.value as Draft[typeof key])} /></label>)}
            <label className={labelClass}><span className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">currency minor unit</span><input className={inputClass} type="number" min="0" max="3" value={draft.currency_minor_unit} onChange={(event) => updateDraft('currency_minor_unit', Number(event.target.value))} /></label>
            <label className={labelClass}><span className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">measurement system</span><select className={inputClass} value={draft.measurement_system} onChange={(event) => updateDraft('measurement_system', event.target.value as Draft['measurement_system'])}><option value="metric">metric</option><option value="imperial">imperial</option></select></label>
            <label className={labelClass}><span className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">effective from</span><input className={inputClass} type="datetime-local" value={draft.effective_from} onChange={(event) => updateDraft('effective_from', event.target.value)} /></label>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <section className="rounded-3xl border border-border bg-surface-subtle p-5">
            ...
            {canWrite && <div className="mt-4 grid gap-3 sm:grid-cols-2"><input className={inputClass} placeholder="city_code, contoh jakarta" value={serviceDraft.city_code} onChange={(event) => setServiceDraft({ ...serviceDraft, city_code: event.target.value })} /><input className={inputClass} placeholder="service_code" value={serviceDraft.service_code} onChange={(event) => setServiceDraft({ ...serviceDraft, service_code: event.target.value })} /><label className="flex items-center gap-2 text-sm text-foreground-muted"><input type="checkbox" checked={serviceDraft.is_enabled} onChange={(event) => setServiceDraft({ ...serviceDraft, is_enabled: event.target.checked })} /> enabled</label><button disabled={availabilityMutation.isPending} onClick={() => availabilityMutation.mutate()} className="rounded-xl bg-primary px-3 py-2 text-sm font-bold text-on-primary disabled:opacity-50">Simpan availability</button></div>}
            <div className="mt-4 space-y-2">{detail.service_availability.map((item) => <div key={`${item.city_code}-${item.service_code}`} className="flex items-center justify-between rounded-xl border border-border bg-surface-subtle px-3 py-2 text-sm"><span className="text-foreground-muted">{item.city_code} · {item.service_code}</span><span className={item.is_enabled ? 'text-success' : 'text-foreground-muted'}>{item.is_enabled ? 'enabled' : 'disabled'}</span></div>)}</div>
          </section>
