                ...
                {revisionsQuery.data?.map((revision) => (
                  <div key={revision.id} className={`p-6 space-y-4 ${selectedId === revision.id ? 'bg-primary/5' : ''}`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                     ...
                     {revision.status === 'published' && selectedId === revision.id && <input aria-label={`Rollback reason for policy ${revision.policy_version}`} value={rollbackReason} onChange={(event) => setRollbackReason(event.target.value)} placeholder="Business reason rollback" className="w-full bg-surface-subtle border border-border rounded-lg p-2.5 text-xs text-foreground-muted" />}
