...
test('orders table exposes header semantics, deliberate overflow and keyboard row activation @a11y', async ({ page }) => {
  ...
  for (const theme of ['light', 'dark'] as const) {
    ...
    await expect.poll(() => themedRow.evaluate((element) => getComputedStyle(element).outlineWidth)).toBe('2px')
    await page.keyboard.press('Enter')
  ...
  await expect.poll(() => refreshedRow.evaluate((element) => getComputedStyle(element).outlineWidth)).toBe('2px')
  await page.keyboard.press('Enter')
