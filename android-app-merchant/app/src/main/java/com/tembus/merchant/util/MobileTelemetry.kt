...
/** Privacy-safe merchant telemetry. Firebase is optional and fail-open. */
class MobileTelemetry(private val context: Context) {
    ...
    fun screenView(screenId: String, outcome: String = "visible") = record(Event.SCREEN_VIEW, mapOf(
        "app" to APP, "screen_id" to classifyScreen(screenId), "market_code" to MARKET,
        "app_version" to BuildConfig.VERSION_NAME.take(MAX_VALUE_LENGTH), "outcome" to boundedOutcome(outcome),
    ))

    fun apiRequest(path: String, statusCode: Int? = null, outcome: String = "completed") = record(Event.API_REQUEST, mapOf(
        "app" to APP, "operation" to operationForPath(path),
        "status_class" to (statusCode?.let(::statusClass) ?: "network_error"), "market_code" to MARKET,
        "app_version" to BuildConfig.VERSION_NAME.take(MAX_VALUE_LENGTH), "outcome" to boundedOutcome(outcome),
    ))
    ...
    fun startup(startedAtElapsedRealtime: Long, mode: String = "resume") {
        ...
        record(Event.APP_START, mapOf(
            "app" to APP, "startup_mode" to if (mode in STARTUP_MODES) mode else "unknown",
            "app_version" to BuildConfig.VERSION_NAME.take(MAX_VALUE_LENGTH), "outcome" to "completed",
    ...
    fun sampleFrameBudget(activity: Activity, surface: String) {
        ...
        val callback = object : Choreographer.FrameCallback {
            override fun doFrame(frameTimeNanos: Long) {
                if (previousFrameNanos != 0L && frameTimeNanos - previousFrameNanos > FRAME_BUDGET_NANOS) jankyFrames++
                ...
                record(Event.FRAME_BUDGET, mapOf(
                    "app" to APP, "surface" to classifyScreen(surface), "device_tier" to deviceTier(activity),
    ...
    private fun record(event: Event, values: Map<String, String>) {
        val bundle = Bundle().apply { values.forEach { (key, value) -> putString(key, value.take(MAX_VALUE_LENGTH)) } }
    ...
    private fun deviceTier(activity: Activity): String {
        ...
        return when { memoryClass < 192 -> "low"; memoryClass >= 384 -> "high"; else -> "mid" }
    }

    companion object {
        private const val TAG = "MobileTelemetry"
        private const val APP = "merchant"
        ...
        private enum class Event(val wireName: String) {
            SCREEN_VIEW("screen_view"), API_REQUEST("api_request"), APP_START("app_start"), FRAME_BUDGET("frame_budget")
        }
        ...
        internal fun statusClass(statusCode: Int): String = when (statusCode) {
            in 200..299 -> "2xx"; in 400..499 -> "4xx"; in 500..599 -> "5xx"; else -> "other"
        }
        internal fun latencyBucket(elapsedMs: Long): String = when {
            elapsedMs <= 500L -> "0_500"; elapsedMs <= 1_500L -> "501_1500";
            elapsedMs <= 3_000L -> "1501_3000"; else -> "3000_plus"
        }
