...
class UpdateManager(
    ...
    suspend fun downloadAndOpenInstaller(version: AppVersion): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                downloadAndOpenInstallerInternal(version, updateUri)
                Result.success(Unit)
    ...
    suspend fun openInstallPermissionSettings(activity: android.app.Activity): Result<Unit> {
    ...
    private suspend fun checkBackendUpdate(): AppVersion? {
        return try {
            val response = apiService.getLatestVersion("merchant", marketCode = DEFAULT_MARKET_CODE)
            ...
            try {
                ...
                httpClient.newCall(request).execute().use { response ->
                    ...
                    releases.asSequence()
                        .filterNot { it.draft }
                        .mapNotNull { release ->
                            val versionCode = MERCHANT_TAG_REGEX
    ...
    private companion object {
        ...
        private val MERCHANT_TAG_REGEX = Regex("""v1\.0\.(\d+)""")
