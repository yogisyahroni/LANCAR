...
CREATE INDEX IF NOT EXISTS idx_trip_share_tokens_order_active
  ON trip_share_tokens(order_id, expires_at DESC)
  WHERE revoked_at IS NULL;

CREATE TABLE IF NOT EXISTS courier_route_snapshots (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  order_id UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
  courier_id UUID REFERENCES users(id) ON DELETE SET NULL,
  route_type VARCHAR(30) NOT NULL DEFAULT 'on_demand',
  distance_km NUMERIC(10,2) NOT NULL DEFAULT 0,
  eta_minutes INT NOT NULL DEFAULT 0,
  polyline JSONB NOT NULL DEFAULT '[]'::jsonb,
  provider VARCHAR(30) NOT NULL DEFAULT 'internal_haversine',
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_courier_route_snapshots_order_created
  ON courier_route_snapshots(order_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS order_stops (
  ...
  recipient_phone_masked VARCHAR(30),
  proof_status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (proof_status IN ('pending', 'verified', 'failed')),
...
CREATE INDEX IF NOT EXISTS idx_order_stops_order_sequence
  ON order_stops(order_id, sequence_no);
...
-- +goose Down
DROP TABLE IF EXISTS courier_tier_configs;
DROP TABLE IF EXISTS courier_incentive_campaigns;
DROP INDEX IF EXISTS idx_order_stops_order_sequence;
DROP TABLE IF EXISTS order_stops;
DROP INDEX IF EXISTS idx_courier_route_snapshots_order_created;
DROP TABLE IF EXISTS courier_route_snapshots;
DROP INDEX IF EXISTS idx_trip_share_tokens_order_active;
DROP TABLE IF EXISTS trip_share_tokens;
DROP INDEX IF EXISTS idx_courier_safety_events_courier;
DROP INDEX IF EXISTS idx_courier_safety_events_open;
DROP TABLE IF EXISTS courier_safety_events;
