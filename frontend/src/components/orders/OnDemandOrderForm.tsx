...
function DimensionScanModal({ isOpen, onClose, onApply }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <FocusTrap active className="relative max-h-[min(90vh,720px)] w-full max-w-2xl">
      <div role="dialog" aria-modal="true" aria-labelledby="dimension-scan-title" tabIndex={-1} className="max-h-[min(90vh,720px)] w-full overflow-y-auto rounded-2xl border border-border bg-background/95 shadow-2xl">
        <div className="flex items-start justify-between border-b border-border p-5"><div><h3 id="dimension-scan-title" className="text-lg font-semibold">Scan Dimensi via Webcam</h3><p className="mt-1 text-sm text-muted-foreground">Letakkan paket dan kartu referensi di area kamera. Hasil bisa dikoreksi manual setelah diterapkan.</p></div><button type="button" onClick={onClose} className="min-h-11 min-w-11 rounded-full p-2 hover:bg-surface-subtle focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="Tutup" title="Tutup"><X className="h-4 w-4" aria-hidden="true" /></button></div>
        <div className="grid gap-5 p-5 md:grid-cols-[1.2fr_.8fr]">
          <div className="relative aspect-video overflow-hidden rounded-xl border border-border bg-scrim">
            ...
            {cameraState !== 'ready' && <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-scrim text-sm text-muted-foreground">{cameraState === 'starting' ? <Loader2 className="h-6 w-6 animate-spin" aria-hidden="true" /> : <Camera className="h-8 w-8"  aria-hidden="true"/>}<span>{cameraMessage}</span>{cameraState === 'blocked' && <button type="button" onClick={() => void startCamera()} className="rounded-md bg-info px-3 py-2 text-xs font-semibold text-on-info hover:bg-info-surface">Coba Kamera Lagi</button>}</div>}
            <div className="pointer-events-none absolute inset-[14%] rounded-xl border-2 border-dashed border-success/70" /><div className="pointer-events-none absolute bottom-3 left-3 rounded-md bg-scrim/60 px-2 py-1 text-xs text-foreground">Align paket di kotak hijau</div>
            {cameraState === 'ready' && <div className="pointer-events-none absolute right-3 top-3 rounded-md bg-success-surface px-2 py-1 text-xs font-semibold text-success">Kamera aktif</div>}
          </div>
          <div className="space-y-4"><div className="rounded-xl border border-border bg-surface-subtle p-4"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Estimasi hasil</p>{result ? <div className="mt-3 grid grid-cols-2 gap-3 text-sm"><div>Panjang: <b>{result.length} cm</b></div><div>Lebar: <b>{result.width} cm</b></div><div>Tinggi: <b>{result.height} cm</b></div><div>Berat: <b>{result.weight_kg} kg</b></div></div> : <p className="mt-3 text-sm text-muted-foreground">Hasil dimensi otomatis belum tersedia. Isi ukuran paket secara manual di form.</p>}</div><button type="button" onClick={runScan} disabled={scanState === 'scanning'} className="w-full rounded-lg bg-info px-4 py-3 text-sm font-semibold text-on-info hover:bg-info-surface disabled:opacity-60">{scanState === 'scanning' ? 'Menganalisis...' : scanState === 'done' ? 'Scan Ulang' : 'Mulai Scan'}</button><button type="button" onClick={() => { if (result) { onApply(result); onClose(); } }} disabled={!result} className="w-full rounded-lg border border-border bg-surface-subtle px-4 py-3 text-sm font-semibold hover:bg-surface-subtle">Terapkan ke Form</button></div>
        </div>
      </div>
      </FocusTrap>
    </div>
