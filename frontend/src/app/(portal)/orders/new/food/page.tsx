...
export default function FoodReorderPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="divide-y divide-border">
          {check.items.map((item) => {
            ...
            return <div key={item.menu_item_id} className={`flex items-center justify-between gap-4 py-4 ${!item.available ? 'opacity-45' : ''}`}>
              ...
              <div className="flex items-center gap-3"><button type="button" aria-label={`Kurangi ${item.item_name}`} title={`Kurangi ${item.item_name}`} disabled={!item.available || quantity === 0} onClick={() => changeQuantity(item.menu_item_id, -1)} className="rounded-lg border border-border p-2 text-foreground disabled:opacity-60"><Minus className="h-4 w-4" aria-hidden="true" /></button><span className="w-5 text-center font-bold text-foreground" aria-label={`Jumlah ${item.item_name}`}>{quantity}</span><button type="button" aria-label={`Tambah ${item.item_name}`} title={`Tambah ${item.item_name}`} disabled={!item.available} onClick={() => changeQuantity(item.menu_item_id, 1)} className="rounded-lg border border-border p-2 text-foreground disabled:opacity-60"><Plus className="h-4 w-4" aria-hidden="true" /></button></div>
            </div>;
          })}
        </div>
      </section>
      ...
      <div className="flex flex-col items-stretch justify-between gap-4 rounded-2xl border border-primary/20 bg-primary/10 p-5 sm:flex-row sm:items-center"><div><p className="text-sm text-muted-foreground">Estimasi subtotal menu</p><p className="text-2xl font-black text-foreground">{formatPrice(total)}</p><p className="text-xs text-muted-foreground">Ongkir dan promo dihitung server saat order dibuat.</p></div><button type="button" onClick={submitReorder} disabled={submitting || !selectedAddress || !check.merchant_open || availableItems.length === 0} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 font-bold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-60">{submitting ? <Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" /> : <ShoppingBag className="h-5 w-5" aria-hidden="true" />} Buat Pesanan Lagi</button></div>
