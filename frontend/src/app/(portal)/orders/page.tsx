  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-foreground/20 via-foreground/20 to-foreground/20 bg-clip-text text-transparent">
            Riwayat Order Anda
          </h1>
      ...
      {/* Tabs */}
      <div role="tablist" aria-label="Filter model order" className="flex bg-muted/60 p-1 rounded-xl border border-border/40 select-none w-fit">
        <button
          role="tab"
      ...
      {/* Filter & Search Panel */}
      <div className="p-6 bg-card/40 backdrop-blur-md rounded-2xl border border-border shadow-sm space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {/* Search bar */}
          <div className="relative col-span-1 md:col-span-2">
            <Search className="absolute left-3.5 top-3 h-4 w-4 text-muted-foreground select-none"  aria-hidden="true"/>
            ...
            <input
              ...
              className="w-full bg-background/50 border border-border rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/40 transition-all"
            />
          </div>

          {/* Status Select */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="order-status" className="text-xs font-semibold text-muted-foreground tracking-wider uppercase flex items-center gap-1">
              <Filter className="h-3 w-3" aria-hidden="true" /> Status
            </label>
            <select
              id="order-status"
              value={status}
              onChange={(e) => updateFilters({ status: e.target.value })}
              className="w-full bg-surface-subtle border border-border rounded-xl px-3 py-2.5 text-sm font-semibold text-foreground-muted focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all cursor-pointer shadow-sm"
            >
              <option value="all" className="bg-surface text-foreground-muted font-medium py-1.5">Semua Status</option>
              <option value="pending" className="bg-surface text-foreground-muted font-medium py-1.5">Pending</option>
              <option value="pending_payment" className="bg-surface text-foreground-muted font-medium py-1.5">Menunggu pembayaran</option>
          ...
          <div className="flex flex-col gap-1.5 hidden md:flex">
            <label htmlFor="order-model" className="text-xs font-semibold text-muted-foreground tracking-wider uppercase flex items-center gap-1">
              <Layers className="h-3 w-3" aria-hidden="true" /> Model
            </label>
            <select
              id="order-model"
              value={model}
              ...
              className="w-full bg-surface-subtle border border-border rounded-xl px-3 py-2.5 text-sm font-semibold text-foreground-muted focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all cursor-pointer shadow-sm"
            >
              <option value="all" className="bg-surface text-foreground-muted font-medium py-1.5">Semua Model</option>
              <option value="p2p" className="bg-surface text-foreground-muted font-medium py-1.5">Instan (P2P)</option>
              <option value="hub_and_spoke" className="bg-surface text-foreground-muted font-medium py-1.5">Ekspedisi (3PL)</option>
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="order-start-date" className="text-xs font-semibold text-muted-foreground tracking-wider uppercase flex items-center gap-1">
              <Calendar className="h-3 w-3" aria-hidden="true" /> Tanggal Mulai
            </label>
            <input
              id="order-start-date"
              type="date"
              ...
              className="w-full bg-background/50 border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none transition-all"
            />
          </div>

          <div className="flex flex-col gap-1.5">
            <label htmlFor="order-end-date" className="text-xs font-semibold text-muted-foreground tracking-wider uppercase flex items-center gap-1">
              <Calendar className="h-3 w-3" aria-hidden="true" /> Tanggal Selesai
            </label>
            <input
              id="order-end-date"
              type="date"
          ...
          <div className="col-span-1 md:col-span-2 flex items-center gap-2">
            ...
            >
              <Search className="h-4 w-4"  aria-hidden="true"/> Cari Sekarang
            </button>
            <button
              onClick={() => {
                setSearch('');
              ...
              className="px-4 py-2.5 bg-surface-subtle hover:bg-surface-subtle border border-border text-foreground rounded-xl text-sm transition-all"
            >
              Reset
            </button>
          </div>
      ...
      {selectedOrders.length > 0 && (
        <div className="p-4 bg-primary/5 border border-primary/20 rounded-xl flex items-center justify-between animate-in slide-in-from-top-2 duration-200">
          ...
          >
            <Download className="h-4 w-4" aria-hidden="true" /> Unduh Resi Terpilih
          </button>
        </div>
      )}
      ...
      <div className="bg-card/40 backdrop-blur-md rounded-2xl border border-border shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center text-muted-foreground flex flex-col items-center justify-center space-y-2">
            <p className="text-lg font-semibold text-foreground">Belum ada order</p>
                ...
                <tr className="border-b border-border bg-surface-subtle">
                  <th scope="col" className="px-5 py-4 w-12 text-center">
                    <input
                      ...
                      className="rounded bg-background/50 border-border focus:ring-primary/40 text-primary"
                    />
                  </th>
                  <th scope="col" className="px-5 py-4 text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                    No Order
                  </th>
                  <th scope="col" className="px-5 py-4 text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                    Resi / AWB
                  ...
                  <th scope="col" className="px-5 py-4 text-xs font-semibold tracking-wider text-muted-foreground uppercase text-center w-24">
                    Aksi
                  </th>
                </tr>
              </thead>
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-sm text-center">
                        <Link
                          ...
                          className="inline-flex items-center justify-center h-8 w-8 rounded-lg bg-surface-subtle hover:bg-primary hover:text-primary-foreground border border-border hover:border-primary transition-all duration-200"
                          title="Lihat Detail"
      ...
      {/* Premium Pagination */}
      {orders.length > 0 && (
        <div className="flex items-center justify-between border border-border rounded-2xl p-4 bg-card/20 backdrop-blur-sm">
