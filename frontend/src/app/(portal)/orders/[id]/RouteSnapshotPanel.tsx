...
export function RouteSnapshotPanel({
  order,
  tracking,
}: {
  order: Order;
  ...
  const snapshot =
    tracking?.order_route_snapshot || order.route_snapshot || null;
  const routePolyline =
  ...
  const provider =
    ...
    "runtime";
  const routeProfile =
    snapshot?.route_profile ||
    tracking?.order_route_profile ||
    order.route_profile ||
    "on-demand";
  ...
  const etaLabel = durationSeconds
    ? `~${Math.ceil(durationSeconds / 60)} menit`
    : snapshot?.eta || tracking?.eta || "ETA diperbarui";
  const svgPath = buildSvgRoute(routePoints);
  const hasProviderFallback =
    !routePolyline || Boolean(snapshot?.fallback_reason);

  const isCancelled = order.status.toLowerCase() === "cancelled";

  return (
    <div
      className={`rounded-2xl border ${isCancelled ? "border-border bg-surface-subtle" : "border-success/15 bg-success/[0.06]"} p-4 shadow-sm`}
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p
            className={`text-xs font-bold uppercase tracking-wider ${isCancelled ? "text-foreground-muted" : "text-success"}`}
          >
            Route snapshot
          </p>
          <h2
            className={`mt-1 text-base font-bold tracking-tight ${isCancelled ? "text-foreground-muted" : "text-foreground"}`}
          >
            {isCancelled ? "Rute dibatalkan" : "Rute pengiriman"}
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {isCancelled ? "-" : distanceLabel} • {isCancelled ? "-" : etaLabel}
          </p>
        </div>
      ...
      <div className="relative h-36 overflow-hidden rounded-2xl border border-border bg-background">
        ...
        <div
          className={`absolute inset-0 bg-[radial-gradient(circle_at_18%_28%,${isCancelled ? "rgba(100,116,139,0.15)" : "rgba(16,185,129,0.18)"},transparent_28%),radial-gradient(circle_at_80%_68%,rgba(249,115,22,0.14),transparent_26%)]`}
        />
        ...
        {isCancelled ? (
          ...
          <svg
            ...
            role="img"
            aria-label="Polyline rute order"
          >
            <path
              d={svgPath}
              fill="none"
            ...
            <circle
              cx="28"
              cy={routePoints.length >= 2 ? "112" : "112"}
              r="10"
              fill="#10b981"
            />
            <circle
              cx="372"
              cy={routePoints.length >= 2 ? "96" : "96"}
              r="10"
              fill="#f97316"
            />
          </svg>
      ...
      {hasProviderFallback && (
        <p className="mt-3 text-xs text-muted-foreground">
          Rute sedang diperbarui. Customer dan kurir tetap memakai estimasi
          backend yang sama sampai provider peta mengirim geometri terbaru.
        </p>
      )}
    </div>
  );
