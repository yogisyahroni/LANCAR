    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div className="space-y-1">
          <Link
            href="/orders"
            className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground transition duration-200"
          >
            <ArrowLeft className="h-4 w-4" aria-hidden="true" /> Kembali ke
            Daftar Order
          </Link>
          <div className="flex items-center gap-3 pt-1">
            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight bg-gradient-to-r from-foreground/20 via-foreground/20 to-foreground/20 bg-clip-text text-transparent">
        ...
        <div className="flex items-center gap-3">
          {order.status.toLowerCase() !== "cancelled" && (
            <>
              ...
              <button
                ...
                className="px-4 py-2.5 bg-surface-subtle hover:bg-surface-subtle border border-border text-foreground rounded-xl text-sm font-medium transition duration-200 flex items-center gap-2"
              >
          ...
          <button
            ...
            className="px-4 py-2.5 bg-error-surface hover:bg-error-surface text-error border border-error rounded-xl text-sm font-medium transition duration-200 flex items-center gap-2"
          >
            <AlertTriangle className="h-4 w-4" aria-hidden="true" />{" "}
            {order.service_category === "on_demand"
      ...
      {order.status.toLowerCase() === "no_courier_found" && (
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl border border-accent bg-gradient-to-br from-accent via-warning to-background p-6 shadow-xl backdrop-blur-md"
        >
          <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-accent-surface blur-3xl" />
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-accent-surface border border-accent text-accent">
                <AlertTriangle
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground max-w-2xl">
                  ...
                  di sekitar area pick up yang menerima pesanan Anda. Anda dapat
                  memulai ulang pencarian kurir sekarang atau membatalkan
                  pesanan dengan pengembalian dana (refund) 100% secara
                  otomatis.
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="col-span-1 lg:col-span-5 space-y-6">
          <div className="relative aspect-square md:aspect-[4/3] lg:aspect-auto lg:h-[620px] bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col justify-between">
            {/* Elegant Header Layer on top of map */}
            <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between p-3.5 bg-background/80 backdrop-blur-md rounded-xl border border-border select-none shadow-lg">
              <div className="flex items-center gap-3">
                <Navigation
                  aria-hidden="true"
                  className={cn(
            ...
            <div className="flex-1 bg-background p-6 flex flex-col justify-center items-center space-y-4 select-none relative overflow-hidden">
              {/* Map Grid Pattern */}
              <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff08_1px,transparent_1px),linear-gradient(to_bottom,#ffffff08_1px,transparent_1px)] bg-[size:24px_24px]" />
              <div className="absolute inset-0 bg-gradient-to-br from-info via-transparent to-info" />

              <Navigation
                aria-hidden="true"
                className="h-10 w-10 text-primary/50 relative z-10"
              />
              <div className="text-center space-y-1 relative z-10">
                <h2 className="font-bold text-foreground tracking-tight">
                  {order.status.toLowerCase() === "cancelled"
                    ? "Peta Pengiriman (Dibatalkan)"
                    : "Peta Pengiriman"}
                </h2>
                  ...
                  {order.status.toLowerCase() === "cancelled"
                    ...
                    : tracking?.location_stale
                      ...
                      : tracking?.location
                        ...
                        : trackingError ||
                          "Lokasi kurir otomatis muncul setelah pekerjaan diterima dan tracking aktif."}
                </p>
              </div>
              ...
              <div className="w-full max-w-xs bg-scrim/40 backdrop-blur-md p-4 border border-border rounded-xl space-y-3.5 relative z-10">
                {tracking?.location &&
                  order.status.toLowerCase() !== "cancelled" && (
                    <>
                      ...
                      <div className="h-8 border-l-2 border-dashed border-border ml-2.5" />
                    </>
                  )}
                <div className="flex items-start gap-3 opacity-90">
                  ...
                  <div>
                    ...
                    <p
                      className="text-xs text-foreground max-w-[190px] truncate"
                ...
                <div className="h-8 border-l-2 border-dashed border-border ml-2.5 opacity-90" />
                <div className="flex items-start gap-3 opacity-90">
                  <MapPin
                    aria-hidden="true"
              ...
              {tracking?.location &&
                order.status.toLowerCase() !== "cancelled" && (
                  ...
                  >
                    <MapPin aria-hidden="true" className="h-4 w-4" /> Buka
                    posisi kurir
                  </a>
                )}
            </div>
            ...
            <div className="p-4 bg-background/90 backdrop-blur-md border-t border-border flex items-center justify-between gap-4">
              {order.status.toLowerCase() === "cancelled" ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ⭐ {order.courier_rating || "5.0"} |{" "}
                        {order.courier_vehicle || "Motor"} (
                        {order.courier_plate || "-"})
                      </p>
                    </div>
                  </div>
                ...
                <div
                  className="space-y-2"
                  aria-busy="true"
                  aria-label="Mencari kurir"
                >
                  <Skeleton className="h-4 w-48 bg-surface-subtle" />
                  <Skeleton className="h-2 w-full bg-surface-subtle" />
                </div>
              )}
            </div>
          </div>
          ...
          {/* Inline Chat Module */}
          <div className="bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm p-4 space-y-4">
            <div className="flex items-center gap-2 border-b border-border pb-3">
              <MessageSquare
                className="h-5 w-5 text-primary"
                aria-hidden="true"
              />
              <h2 className="text-sm font-bold">Obrolan dengan Kurir</h2>
            </div>
            <div
              ref={chatScrollRef}
              className="h-[210px] bg-background/40 border border-border rounded-xl p-3.5 overflow-y-auto space-y-3.5 scroll-smooth"
            >
              {chatsLoading && chatMessages.length === 0 ? (
                ...
                >
                  <Skeleton className="h-8 w-4/5 bg-surface-subtle" />
                  <Skeleton className="ml-auto h-8 w-3/5 bg-surface-subtle" />
                  <Skeleton className="h-8 w-2/3 bg-surface-subtle" />
                </div>
            ...
            <AnimatePresence>
              {previewImage && (
                <motion.div
                  ...
                  className="p-3 bg-muted/20 border border-border rounded-xl flex items-center gap-3 mb-2"
                >
            ...
            <form onSubmit={handleSendMessage} className="flex gap-2">
              ...
              <input
                type="text"
                aria-label="Pesan dukungan order"
                value={chatInput}
        ...
        <div className="col-span-1 lg:col-span-7 space-y-6">
          {/* Order Data Summary */}
          <div className="p-6 bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm space-y-6">
            <div className="border-b border-border pb-4">
              <h2 className="text-lg font-bold text-foreground tracking-tight flex items-center gap-2">
                <Calendar className="h-5 w-5 text-primary" aria-hidden="true" />{" "}
                Rincian Order & Pengiriman
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div className="space-y-1 md:col-span-2">
                ...
                <p className="text-sm text-foreground-secondary italic leading-relaxed">
                  {order.customer_notes || "Tidak ada catatan khusus."}
                </p>
              </div>
            </div>
          </div>
          ...
          {hasPackageDetails && (
            <div className="p-6 bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm space-y-5">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="rounded-xl border border-border bg-background/40 p-4">
                  ...
                  <p className="mt-1 text-sm font-semibold text-foreground">
                    {packageDetails.category || packageDetails.size_tier || "-"}
                  </p>
                </div>
                <div className="rounded-xl border border-border bg-background/40 p-4">
                  <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                ...
                <div className="rounded-xl border border-border bg-background/40 p-4">
                  ...
                  <p className="mt-1 text-sm font-semibold text-foreground">
                    {packageDetails.dimensions_scanned
                      ? "Scan selesai"
                      : "Manual / tidak wajib scan"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="p-6 bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm space-y-4">
            <h2 className="text-base font-bold text-foreground tracking-tight flex items-center gap-2 border-b border-border pb-3">
              Kalkulasi Biaya
            </h2>
            <div className="space-y-2.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatPrice(order.base_price_idr || 0)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">
                  Surge & Volumetrik Surcharge
                </span>
                <span className="font-medium">
                  {formatPrice(order.volumetric_surcharge_idr || 0)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Asuransi barang</span>
                <span className="font-medium">
                  {formatPrice(order.insurance_premium_idr || 0)}
                </span>
              </div>
              <div className="pt-2 border-t border-border flex justify-between font-bold text-base bg-surface-subtle p-3 rounded-xl">
                <span>TOTAL HARGA</span>
                <span className="text-primary">
                  {formatPrice(order.total_price_idr || 0)}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="p-6 bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm space-y-5">
              <div className="flex items-center justify-between gap-3 border-b border-border pb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {(order.status.toLowerCase() === "completed" ||
                    ...
                    order.status.toLowerCase() === "cancelled") && (
                    <Link
                      href={`/orders/new/food?orderId=${encodeURIComponent(order.id)}`}
              ...
              <div className="space-y-3">
                {foodItems.map((item: any, index: any) => {
                  const itemName =
                    item.name || item.item_name || "Item makanan";
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-foreground font-medium">
                      Sistem sedang memproses booking Anda.
                    </p>
                  </div>
                </div>
              ) : (
                  ...
                  <div key={event.id} className="relative">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <h3 className="text-sm font-bold tracking-tight text-foreground capitalize">
            ...
            <div className="p-6 bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm space-y-6">
              ...
              <div className="space-y-3">
                {carrierEvents.map((event: any) => {
                  const statusPresentation = presentCarrierStatus(
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {formatDate(event.occurred_at || event.received_at)}
                        </span>
                      </div>
                      {statusPresentation.isUnknown && (
                      ...
                      {providerStatus &&
                        providerStatus.toUpperCase() !==
                          ...
                          <p className="mt-2 text-sm text-foreground-secondary">
                            Status {event.provider}: {providerStatus}
                          </p>
                        )}
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                        {event.provider_status_code && (
                          <span>Kode: {event.provider_status_code}</span>
                        )}
                        {event.provider_location && (
                          <span>Lokasi: {event.provider_location}</span>
                        )}
                      </div>
                    </div>
                  );
          ...
          <div className="p-6 bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm space-y-5">
            <div className="flex items-center justify-between gap-3 border-b border-border pb-3">
              <h2 className="text-base font-bold text-foreground tracking-tight flex items-center gap-2">
                <ImageIcon
                  ...
                  aria-hidden="true"
                />{" "}
                Bukti pickup, POD, dan pembatalan
              </h2>
              <span className="rounded-full border border-border bg-surface-subtle px-3 py-1 text-xs font-semibold text-muted-foreground">
                {proofs.length} bukti
              </span>
            ...
            {proofs.length === 0 ? (
              <div className="rounded-xl border border-border bg-background/40 p-4 text-sm text-muted-foreground">
                Bukti operasional akan muncul setelah kurir melakukan scan
                pickup, foto barang, POD, atau pembatalan sebelum pickup.
              </div>
            ) : (
              <div className="space-y-4">
                {proofGroups.cancellation.map((proof: any) => (
                  <div
                    key={proof.id}
                    className="rounded-xl border border-error bg-error-surface p-4"
                  >
                    <div className="flex items-start gap-3">
                      <AlertTriangle
                        className="mt-0.5 h-5 w-5 text-error"
                        aria-hidden="true"
                      />
                      <div className="flex-1">
                        <p className="text-sm font-bold text-error">
                          {proof.proof_label || "Bukti pembatalan pickup"}
                        </p>
                    ...
                    {proof.photo_url && (
                      <button
                        type="button"
                        onClick={() =>
                          setActivePhoto(uploadUrl(proof.photo_url))
                        }
                        className="mt-3 overflow-hidden rounded-xl border border-error"
                      >
                        <img
                          src={uploadUrl(proof.photo_url)}
                          alt={proof.proof_label || "Bukti pembatalan"}
                  ...
                  return (
                    ...
                    >
                      <div className="mb-3 flex items-center gap-2">
                        ...
                        />
                        <p className="text-sm font-bold text-foreground">
                          {group === "pickup" ? "Bukti pickup" : "Bukti POD"}
                        </p>
                      </div>
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        {groupProofs.map((proof: any) => {
                          const imageUrl = uploadUrl(
                            proof.photo_url || proof.image_urls?.[0],
                          );
                          return (
                            <div
                              key={proof.id}
                              ...
                              {imageUrl ? (
                                ...
                                >
                                  <img
                                    src={imageUrl}
                                    alt={
                                      proof.proof_label || "Bukti pengiriman"
                                    }
                                    className="h-36 w-full object-cover transition hover:opacity-90"
                                  />
                                </button>
                              ) : (
                                <div className="mt-3 rounded-lg border border-border bg-surface-subtle px-3 py-2 text-xs text-muted-foreground">
          ...
          {(serviceProofs.length > 0 || serviceReportNotes) && (
            <div className="p-6 bg-card/40 backdrop-blur-md border border-border rounded-2xl shadow-sm space-y-5">
              <div className="flex items-center justify-between gap-3 border-b border-border pb-3">
                <h2 className="text-base font-bold text-foreground tracking-tight flex items-center gap-2">
                  <ImageIcon className="h-5 w-5 text-primary" aria-hidden="true" /> Bukti layanan
                </h2>
                <span className="rounded-full border border-border bg-surface-subtle px-3 py-1 text-xs font-semibold text-muted-foreground">
      ...
      <AnimatePresence>
        {activePhoto && (
          <motion.button
            type="button"
            className="fixed inset-0 z-50 flex items-center justify-center bg-scrim/80 p-6 backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <img
              ...
              className="max-h-[86vh] max-w-[92vw] rounded-2xl border border-border object-contain shadow-2xl"
            />
          </motion.button>
        )}
      </AnimatePresence>
      ...
      <DisputeModal
        ...
        onSuccess={() => {
          addNotification({
            title: "Terkirim",
            message: "Laporan Anda telah kami terima dan akan segera diproses.",
            type: "success",
          });
        }}
      />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-scrim/60 backdrop-blur-sm animate-in fade-in">
            <FocusTrap active>
              <motion.div
                ...
                className="relative w-full max-w-md overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-2xl backdrop-blur-xl"
              >
                <div className="flex items-center justify-between border-b border-border pb-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-error-surface border border-error text-error">
                      <AlertTriangle className="h-5 w-5" aria-hidden="true" />
                    </div>
                    <div>
                      <h3
                      ...
                      >
                        Pengembalian dana 100% otomatis
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="my-5 space-y-3 text-sm text-muted-foreground">
                  ...
                  <div className="rounded-xl border border-border bg-surface/[0.02] p-3.5 space-y-2">
                    ...
                    <p className="text-xs leading-relaxed text-foreground-muted">
                      Proses refund ke metode pembayaran awal atau saldo dompet
                      Anda akan diproses secara instan oleh sistem.
                    </p>
                  </div>
                </div>
