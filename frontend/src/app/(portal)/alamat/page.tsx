    ...
    <div className="space-y-6 select-none">
      ...
      {importPreview && (
        <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5 space-y-4" role="dialog" aria-label="Preview import alamat">
          ...
          <div className="flex justify-end gap-3"><button type="button" onClick={() => setImportPreview(null)} className="px-4 py-2 text-xs font-semibold text-muted-foreground">Batal</button><button type="button" disabled={isImporting || !importPreview.some(row => row.payload)} onClick={confirmAddressImport} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-on-primary disabled:opacity-60"><CheckCircle className="h-3.5 w-3.5" aria-hidden="true" />{isImporting ? 'Menyimpan...' : 'Import baris valid'}</button></div>
        </div>
      )}
