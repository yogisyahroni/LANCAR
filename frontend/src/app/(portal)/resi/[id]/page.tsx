...
export default function ResiDetailPage({
  params,
}: {
  ...
  const fetchOrder = async () => {
    try {
      ...
      if (res.data && res.data.success && res.data.order) {
        ...
        if (res.data.order.awb_number) {
          try {
            const tmplRes = await api.get(
              `/resi/render/${res.data.order.awb_number}`,
            );
        ...
        addNotification({
          title: "Gagal",
          message: "Resi tidak ditemukan pada database.",
          type: "error",
        });
      }
    } catch (error) {
      clientLog.error("Failed to fetch customer order receipt", {
        error,
      ...
      addNotification({
        title: "Gagal",
        message: "Gagal mengambil detail resi dari server.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleCopyPublicLink = () => {
    ...
    addNotification({
      title: "Berhasil",
      message: "Tautan resi berhasil disalin ke clipboard.",
      type: "success",
    });
  ...
  const resolveValue = (val: string, currentOrder: Order) => {
    ...
    return val
      ...
      .replace(
        /{{provider_name}}/g,
        currentOrder.logistics_provider || currentOrder.model || "",
      )
      .replace(
        /{{service_type}}/g,
        currentOrder.logistics_service_type || currentOrder.model || "",
      )
      .replace(
        /{{service_name}}/g,
        currentOrder.service_snapshot?.service_name || currentOrder.model || "",
      )
      .replace(/{{total_price}}/g, formatIDR(currentOrder.total_price_idr || 0))
      .replace(
        /{{total_price_idr}}/g,
        formatIDR(currentOrder.total_price_idr || 0),
      )
      ...
      .replace(
        /{{total_weight}}/g,
        String((currentOrder as any).total_weight ?? "-"),
      )
      .replace(
        /{{total_items}}/g,
        String((currentOrder as any).total_items ?? "-"),
      )
      .replace(
        /{{order_id}}/g,
      ...
      .replace(
        /{{tracking_url}}/g,
        `${window.location.origin}/resi/${currentOrder.id}`,
      );
  };
  ...
  return (
    <div className="space-y-6 select-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 print:hidden">
        <h1 className="text-2xl font-bold tracking-tight text-foreground">
          Detail Resi {order.order_number}
        </h1>
        <Link
          href="/resi"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Share2 className="h-3.5 w-3.5" aria-hidden="true" /> WhatsApp
          </button>
          <button
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-4 py-2 bg-primary hover:bg-primary/90 text-on-primary text-xs font-bold rounded-xl shadow-md shadow-primary/20 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer select-none"
          >
            <Printer className="h-3.5 w-3.5"  aria-hidden="true"/> Cetak Resi
          </button>
        </div>
      </div>
      ...
      >
        {templateData && elements.length > 0 ? (
          <div
            className="relative bg-surface shadow-sm border border-border overflow-hidden mx-auto print:shadow-none print:border-none"
              ...
              >
                ...
                {el.type === "logo" &&
                  (el.value && el.value.startsWith("http") ? (
                    <img
                      src={resolveValue(el.value, order)}
                      alt="Logo"
                      style={{
                        width: el.width || 100,
                        height: el.height || 32,
          ...
          <div className="bg-surface p-8 border border-border rounded-2xl">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-dashed border-border pb-6 gap-6">
              <div>
                <h2 className="text-2xl font-black tracking-tighter text-primary">
                  TEMBUS DELIVERY
                </h2>
                <p className="text-xs text-foreground-muted font-medium select-none">
                  PT TEMBUS LINTAS TEKNOLOGI
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 my-8 flex-1">
              ...
              <div className="md:col-span-2 space-y-6">
                <div>
                  <h3 className="text-xs font-bold uppercase text-foreground-muted flex items-center gap-1 select-none">
                    <User className="h-3.5 w-3.5 text-primary" aria-hidden="true" /> Pengirim
                    (Sender)
                  </h3>
                  <p className="text-sm font-extrabold text-foreground-muted mt-1">
                    {order.sender_name || "Customer"}
                  </p>
                ...
                <div className="border-t border-border pt-4">
                  <h3 className="text-xs font-bold uppercase text-foreground-muted flex items-center gap-1 select-none">
                    <User className="h-3.5 w-3.5 text-success" aria-hidden="true" /> Penerima
                    (Recipient)
                  </h3>
                  <p className="text-sm font-extrabold text-foreground-muted mt-1">
                  ...
                  <div className="flex items-start gap-2 mt-2 select-none">
                    <MapPin className="h-4 w-4 shrink-0 text-success mt-0.5" aria-hidden="true" />
                    <p className="text-xs text-foreground-muted leading-relaxed font-medium">
              ...
              <div className="bg-surface-subtle p-6 rounded-2xl border border-border flex flex-col items-center justify-center text-center select-none">
                <QRCodeSVG
            ...
            {(order.carrier_events || []).length > 0 && (
              <div className="border-t border-border pt-5 mt-5">
                <h3 className="text-xs font-bold uppercase text-foreground-muted">
                ...
                <div className="mt-3 space-y-2">
                  {order.carrier_events?.map((event) => (
                    ...
                    >
                      ...
                      {presentCarrierStatus(event.canonical_status)
                        .isUnknown && (
                        <p className="mt-1">
                          {
                            presentCarrierStatus(event.canonical_status)
                      ...
                      {event.provider_status && (
                        <p className="mt-1">
                          Status asli: {event.provider_status}
                        </p>
                      )}
                      {event.provider_status_description && (
                        <p>{event.provider_status_description}</p>
                      )}
                      {(event.provider_status_code ||
                        ...
                        <p className="mt-1 text-foreground-muted">
                          {[
                            event.provider_status_code &&
                              `Kode ${event.provider_status_code}`,
                            event.provider_location &&
                              `Lokasi ${event.provider_location}`,
                          ]
                            .filter(Boolean)
                            .join(" · ")}
                        </p>
            ...
            <div className="border-t border-border pt-4 flex flex-col md:flex-row md:items-center justify-between text-foreground-muted text-[10px] select-none gap-2">
              ...
              <p className="font-mono select-none font-bold">
                TEMBUS v1.0 • System Generated Receipt
              </p>
            </div>
          </div>
        )}
