...
- [ ] **Marketplace parity PART W–AG — payment, safety, mobile, security, reputation/CRM and reality gates**
  - Status 2026-09-13: staging remains on this device with the canonical boundaries `api.bawain.my.id` (API), `app.bawain.my.id` (customer), `admin.bawain.my.id` (admin), and `bawain.my.id` (landing). Docker and the cloudflared tunnel were restarted and public roots returned 200. SAFE-2026-003 now has explicit TOTP/idempotent pause/hold/reassign/release controls, an order-state guard, and a durable courier offline incident queue; targeted tests, Admin build, Android unit build, migration state and a rollback-safe staging SQL probe passed. The remaining W–AG checklist items are still open and must not be treated as complete.
