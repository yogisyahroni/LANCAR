...
- [ ] **Marketplace parity PART W–AG — payment, safety, mobile, security, reputation/CRM and reality gates**
  - Status 2026-09-14: staging remains on this device with the canonical boundaries `api.bawain.my.id` (API), `app.bawain.my.id` (customer), `admin.bawain.my.id` (admin dashboard), and `bawain.my.id` (landing). Admin was rebuilt/recreated from `docker-compose.yml` staging and verified healthy; all four public roots returned 200, and the Admin UI visibly served PLATFORM OPERATIONS from `admin.bawain.my.id`. SAFE-2026-010 now has all seven mandatory scenarios evidenced by repeatable public staging drills, including Towing/Paket reporting, protected evidence denial, Ops critical/SLA visibility, SOS fallback, kill-switch preservation and socket reconnect. The remaining W–AG checklist items are still open and must not be treated as complete.
  ...
  - Evidence: `docs/task-evidence/PAYPLAT-2026-001.md` through `REALITY-2026-012.md`; latest validator must pass before any additional master checklist item is checked.
...
- [ ] **Customer Google Login + Zenziva OTP Infrastructure** - detail eksekusi ada di [task-customer-google-auth-zenziva-otp.md](task-customer-google-auth-zenziva-otp.md); scope customer web, Android customer app, auth-service, API gateway, database, trusted device, dan provider OTP WA/SMS. Status: backend `google_auth_service.go` (1053 lines) sudah kompleks — ada OAuth URL generation, ID token verification, identity lookup (3 cases: existing user, email match, new user), OTP send (WhatsApp primary + SMS fallback), dry-run provider, anti-ternak + device fingerprinting + audit logging. ✅ Fix code-level: hardcoded `YOUR_GOOGLE_WEB_CLIENT_ID` placeholder diganti error bila belum configured (commit `ddaf629`). ⏳ Tersisa: Zenziva provider config (skip dulu, butuh keputusan provider), Android Google button (masih placeholder), env config `GOOGLE_CUSTOMER_WEB_CLIENT_ID`/`GOOGLE_CUSTOMER_ANDROID_CLIENT_ID` di server, end-to-end flow verification.
