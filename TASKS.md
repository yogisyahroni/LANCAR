...
- [ ] **Customer Google Login + Zenziva OTP Infrastructure** - detail eksekusi ada di [task-customer-google-auth-zenziva-otp.md](task-customer-google-auth-zenziva-otp.md); scope customer web, Android customer app, auth-service, API gateway, database, trusted device, dan provider OTP WA/SMS. Status: awal implementasi framework OTP + Google login di repo sudah ada (endpoints, feature flag, trusted device table, Android API contract, customer web login/register pakai OTP), tetapi butuh konfigurasi Zenziva provider API keys (WhatsApp OTP primary, SMS fallback) untuk mengaktifkan end-to-end OTP flow. External config dibutuhkan: Zenziva API credentials.
