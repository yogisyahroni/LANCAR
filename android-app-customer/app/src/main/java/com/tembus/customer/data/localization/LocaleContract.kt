...
/** Shared locale rules for customer UI, persisted settings, and user-facing formatting. */
object LocaleContract {
    ...
    fun normalizeLanguageTag(value: String?): String = when {
        value?.trim()?.lowercase(Locale.ROOT)?.startsWith("en") == true -> "en-US"
    ...
    fun fallbackChain(requested: String?, marketDefault: String? = DEFAULT_LANGUAGE_TAG): List<String> {
        ...
        return listOf(request, request.substringBefore('-'), market, market.substringBefore('-'), DEFAULT_LANGUAGE_TAG, "id")
            .distinct()
    }

    fun isRtl(value: String?): Boolean = setOf("ar", "fa", "he", "ur", "ps", "ku", "dv", "yi")
        .contains(value?.trim()?.replace('_', '-')?.substringBefore('-')?.lowercase(Locale.ROOT))
}
...
object LocaleFormatters {
    ...
    fun address(
        ...
        val values = if (LocaleContract.localeFor(languageTag).language == "en") {
            listOf(street, city, region, postalCode, district, country)
        } else {
            listOf(street, district, city, region, postalCode, country)
        }
        val parts = values.filter { !it.isNullOrBlank() }.map { it!!.trim() }
    ...
    private fun format(epochMillis: Long, languageTag: String?, timeZoneId: String?, style: Int): String {
        val formatter = if (style == STYLE_DATE) {
            DateFormat.getDateInstance(DateFormat.MEDIUM, LocaleContract.localeFor(languageTag))
        } else if (style == STYLE_TIME) {
            DateFormat.getTimeInstance(DateFormat.SHORT, LocaleContract.localeFor(languageTag))
        } else {
            DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.SHORT, LocaleContract.localeFor(languageTag))
        }
