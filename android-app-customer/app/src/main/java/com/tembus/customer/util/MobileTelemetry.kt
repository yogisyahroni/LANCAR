...
/**
 * Privacy-safe client telemetry for the governed mobile event contract.
 *
 * Firebase Analytics is optional: missing project configuration disables the
 * sink without blocking startup, order, payment, safety or recovery flows.
 */
@Singleton
class MobileTelemetry @Inject constructor(
    ...
    private val analytics: FirebaseAnalytics? by lazy {
        runCatching {
            if (FirebaseInitializer.isInitialized(context)) {
    ...
    fun apiRequest(path: String, statusCode: Int? = null, outcome: String = "completed") {
        val statusClass = statusCode?.let(::statusClass) ?: "network_error"
        record(
            Event.API_REQUEST,
            mapOf(
                "app" to APP,
    ...
    /** Samples 30 choreographer frames asynchronously and never blocks UI. */
    fun sampleFrameBudget(activity: Activity, surface: String) {
        ...
        val callback = object : Choreographer.FrameCallback {
            override fun doFrame(frameTimeNanos: Long) {
                ...
                Log.d(TAG, "frame_budget sampled=${(System.nanoTime() - startedAt) / 1_000_000L}ms")
            }
        }
    ...
    private fun deviceTier(activity: Activity): String {
        val memoryClass = (activity.getSystemService(Context.ACTIVITY_SERVICE) as? ActivityManager)
            ?.memoryClass ?: 0
        return when {
            memoryClass < 192 -> "low"
    ...
    companion object {
        ...
        private const val APP = "customer"
        ...
        private val ALLOWED_EVENTS = setOf(
            "screen_view", "api_request", "app_start", "frame_budget", "network_recovery",
        )
        private val ALLOWED_DIMENSIONS = setOf(
            "app", "screen_id", "market_code", "app_version", "outcome", "operation",
            "status_class", "startup_mode", "latency_bucket", "surface", "device_tier",
        )
