...
jest.mock('./controllers', () => ({
  ...
  getMobileCourierMarketEligibility: jest.fn((req, res) => res.status(200).json({ success: true, data: {} })),
  createMobileCourierMarketChangeRequest: jest.fn((req, res) => res.status(201).json({ success: true, data: {} })),
  createMobileCourierCrossBorderEligibility: jest.fn((req, res) => res.status(201).json({ success: true, data: {} })),
  listAdminCourierMarketConfigs: jest.fn((req, res) => res.status(200).json({ success: true, data: [] })),
  listAdminCourierMarketChangeRequests: jest.fn((req, res) => res.status(200).json({ success: true, data: [] })),
  createAdminCourierMarketConfig: jest.fn((req, res) => res.status(201).json({ success: true, data: {} })),
  updateAdminCourierMarketConfig: jest.fn((req, res) => res.status(200).json({ success: true, data: {} })),
  reviewAdminCourierMarketChangeRequest: jest.fn((req, res) => res.status(200).json({ success: true, data: {} })),
  ...
  retireAdminExperienceManifest: jest.fn((req, res) => res.status(200).json({ success: true, data: {} })),
  ...
  listAdminExperienceDeepLinkRegistry: jest.fn((req, res) => res.status(200).json({ success: true, data: [] })),
...
describe('Admin Service Routes', () => {
  ...
  it('maps every Admin Experience contract action to an authenticated controller', async () => {
    ...
    const requests = [
      ...
      request(app).post(`/admin/experience/manifests/${manifestId}/retire`).send({ reason: 'Retire campaign' }).set(headers),
      ...
      request(app).get('/admin/experience/deep-link-registry').query(scope).set(headers),
