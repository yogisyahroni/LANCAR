...
func (s *orderServiceImpl) CreateOrder(ctx context.Context, userID string, req domain.CreateOrderRequest) (*domain.Order, error) {
	...
	order := &domain.Order{
		...
		CurrencyMinorUnit:        estimate.CurrencyMinorUnit,
		BasePriceMinor:           estimate.BasePriceIDR,
		DistanceFeeMinor:         estimate.DistanceFeeIDR,
		VolumetricSurchargeMinor: estimate.VolumetricSurchargeIDR,
		DynamicPriceMinor:        estimate.DynamicPriceIDR,
		DiscountMinor:            estimate.DiscountIDR,
		PlatformFeeMinor:         estimate.PlatformFeeIDR,
		TotalPriceMinor:          estimate.TotalPriceMinor,
	...
	if err == nil {
		...
		order.DPPMinor = taxSnapshot.DPPMinor
		order.PPNMinor = taxSnapshot.PPNMinor
		order.TaxRuleVersion = taxSnapshot.TaxRuleVersion
		order.TaxJurisdiction = taxSnapshot.TaxJurisdiction
