...
func (r *carrierHandoffRepoStub) GetAWBAttemptByOrder(context.Context, string) (*domain.AWBAttempt, error) {
	return r.attempt, nil
}
func (r *carrierHandoffRepoStub) GetAWBAttemptByAWB(context.Context, string, string) (*domain.AWBAttempt, error) {
	return r.attempt, nil
}
...
type carrierHandoffAWBStub struct {
	calls int
	err   error
}
...
func (*carrierHandoffAWBStub) CheckTariff(context.Context, domain.CheckTariffRequest) (*domain.CheckTariffResponse, error) {
	return nil, nil
}

type carrierHandoffConfigStub struct{}

func (carrierHandoffConfigStub) GetConfig(context.Context, string) (*domain.SystemConfig, error) {
	return nil, nil
}
...
func (carrierHandoffConfigStub) GetStringConfig(_ context.Context, key, fallback string) string {
	switch key {
	case "awb_jne_first_mile_mode":
		return "lancar_pickup"
	case "awb_jne_first_mile_modes":
		return `["lancar_pickup","provider_pickup","customer_dropoff"]`
	default:
		return fallback
	}
}
...
func TestCarrierHandoffLancarPickupRequiresProof(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected missing location/evidence to be rejected")
	}
	...
	if err != nil {
		t.Fatalf("record proof handoff: %v", err)
	}
	if handoff.FirstMileMode != domain.FirstMileLancarPickup || repo.handoff == nil {
