...
type finalRoadsideEvidence struct {
	ReportID  string
	OrderID   string
	CourierID string // canonical users.id, not courier_profiles.id
}
...
func (r *roadsideAftercareRepo) SubmitClaim(ctx context.Context, req *domain.SubmitRoadsideClaimRequest, customerID string) (*domain.RoadsideServiceClaim, error) {
	...
	defer tx.Rollback()
	if _, err = tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtext($1))`, "roadside-aftercare:"+req.OrderID); err != nil {
		return nil, err
	}

	if old, hash, e := claimByKey(ctx, tx, customerID, req.IdempotencyKey); e == nil {
		if old.OrderID == req.OrderID && hash == req.RequestFingerprint {
			return old, nil
		}
	...
	err = tx.QueryRowContext(ctx, `INSERT INTO roadside_service_claims
		(order_id,customer_id,courier_id,report_id,report_snapshot,report_snapshot_hash,report_snapshot_bytes,issue_type,description,idempotency_key,request_hash,correlation_id)
		...
		req.OrderID, customerID, ev.CourierID, ev.ReportID, snapshot, hash, snapshot, req.IssueType, req.Description, req.IdempotencyKey, req.RequestFingerprint, req.CorrelationID,
	...
	if err != nil {
		return nil, fmt.Errorf("insert roadside claim: %w", err)
	}
	if err = aftercareAudit(ctx, tx, customerID, "roadside_claim.submitted", item.ID, item.OrderID, item.ReportID, item.ReportSnapshotHash); err != nil {
		return nil, err
	}
...
func (r *roadsideAftercareRepo) SubmitRating(ctx context.Context, req *domain.SubmitRoadsideRatingRequest, customerID string) (*domain.RoadsideServiceRating, error) {
	...
	var exists bool
	if err = tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM roadside_service_ratings WHERE order_id=$1)`, req.OrderID).Scan(&exists); err != nil {
		return nil, err
	}
	if exists {
		return nil, domain.ErrRoadsideAftercareConflict
	}
	...
	err = tx.QueryRowContext(ctx, `INSERT INTO roadside_service_ratings
		(order_id,customer_id,courier_id,report_id,report_snapshot,report_snapshot_hash,report_snapshot_bytes,overall_rating,technician_quality_rating,comment,idempotency_key,request_hash,correlation_id)
		VALUES($1,$2,$3,$4,$5::jsonb,$6,$7,$8,$9,$10,$11,$12,NULLIF($13,''))
		...
		req.OrderID, customerID, ev.CourierID, ev.ReportID, snapshot, hash, snapshot, req.OverallRating, req.TechnicianQualityRating, req.Comment, req.IdempotencyKey, req.RequestFingerprint, req.CorrelationID,
	...
	if err != nil {
		return nil, fmt.Errorf("insert roadside rating: %w", err)
	}
	if err = aftercareAudit(ctx, tx, customerID, "roadside_rating.submitted", item.ID, item.OrderID, item.ReportID, item.ReportSnapshotHash); err != nil {
		return nil, err
	}
	if err = tx.Commit(); err != nil {
		return nil, err
...
func finalEvidence(ctx context.Context, tx *sql.Tx, orderID, customerID string) (*finalRoadsideEvidence, []byte, string, error) {
	...
	var snapshot string
	err := tx.QueryRowContext(ctx, `SELECT r.id::text,o.id::text,cp.user_id::text,to_jsonb(r)::text
        FROM orders o
        JOIN order_legs ol ON ol.order_id=o.id AND ol.leg_number=1
        JOIN courier_profiles cp ON cp.user_id=ol.courier_id
        JOIN tambal_ban_reports r ON r.order_id=o.id AND r.courier_id=cp.id
        ...
        ORDER BY r.created_at ASC LIMIT 1 FOR SHARE OF o,r`, orderID, customerID).Scan(&e.ReportID, &e.OrderID, &e.CourierID, &snapshot)
	if errors.Is(err, sql.ErrNoRows) {
		var owns bool
		if qerr := tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM orders WHERE id=$1 AND customer_id=$2)`, orderID, customerID).Scan(&owns); qerr != nil {
			return nil, nil, "", qerr
		}
		if !owns {
			return nil, nil, "", domain.ErrRoadsideAftercareForbidden
		}
...
func ratingByKey(ctx context.Context, tx *sql.Tx, customerID, key string) (*domain.RoadsideServiceRating, string, error) {
	i := &domain.RoadsideServiceRating{}
	var h string
