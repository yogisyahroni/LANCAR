...
func (r *postgresRepo) GetByID(ctx context.Context, id string) (*domain.Order, error) {
	query := `SELECT 
				...
				COALESCE(o.distance_km, 0), COALESCE(o.included_distance_km, 0), COALESCE(o.distance_fee_idr, 0), COALESCE(o.volumetric_weight_kg, 0),
				COALESCE(o.base_price_idr, 0), COALESCE(o.volumetric_surcharge_idr, 0),
				COALESCE(o.dynamic_price_idr, 0), COALESCE(o.surge_fee_idr, 0), COALESCE(o.discount_idr, 0), COALESCE(o.promo_code, ''), COALESCE(o.promo_sponsor, 'platform'),
				...
				COALESCE(o.total_price_idr, 0), COALESCE(o.handover_token, ''), o.dispatch_expiry, o.batch_id, o.sequence_no,
...
func (r *postgresRepo) GetByOrderNumber(ctx context.Context, orderNumber string) (*domain.Order, error) {
	query := `SELECT 
				...
				COALESCE(distance_km, 0), COALESCE(base_price_idr, 0), COALESCE(volumetric_surcharge_idr, 0),
				COALESCE(dynamic_price_idr, 0), COALESCE(total_price_idr, 0), COALESCE(handover_token, ''), dispatch_expiry, COALESCE(batch_id::text, ''), COALESCE(sequence_no, 0),
...
func (r *postgresRepo) ListByUserID(ctx context.Context, userID string, filter map[string]interface{}) ([]*domain.Order, error) {
	query := `SELECT 
				...
				COALESCE((SELECT ol.courier_id::text FROM order_legs ol WHERE ol.order_id = orders.id AND ol.leg_number = 1 LIMIT 1), ''),
				created_at, updated_at,
				COALESCE(currency_code, 'IDR'), COALESCE(currency_minor_unit, 0), COALESCE(base_price_minor, 0), COALESCE(distance_fee_minor, 0),
				COALESCE(volumetric_surcharge_minor, 0), COALESCE(dynamic_price_minor, 0), COALESCE(discount_minor, 0), COALESCE(insurance_premium_minor, 0),
				COALESCE(platform_fee_minor, 0), COALESCE(promo_subsidy_minor, 0), COALESCE(total_price_minor, 0), COALESCE(dpp_minor, 0), COALESCE(ppn_minor, 0),
				COALESCE(tax_rule_version, ''), COALESCE(tax_jurisdiction, '')
