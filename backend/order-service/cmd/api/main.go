...
func main() {
	...
	roadsideSettlementSvc.(interface {
		SetTaxPolicy(domain.RoadsideSettlementTaxPolicy)
	}).SetTaxPolicy(service.NewRoadsideSettlementTaxPolicy(taxRepo, configRepo))
	roadsideCollectionRepo := repository.NewRoadsideCollectionRepository(db)
	roadsideCollectionSvc := service.NewRoadsideCollectionService(roadsideCollectionRepo, paymentGw, taxSvc, configRepo)
	paymentSvc.SetRoadsideCollectionService(roadsideCollectionSvc)
	...
	roadsideCollectionHandler := handler.NewRoadsideCollectionHandler(roadsideCollectionSvc)
	roadsideAftercareSvc := service.NewRoadsideAftercareService(repository.NewRoadsideAftercareRepository(db))
	roadsideAftercareHandler := handler.NewRoadsideAftercareHandler(roadsideAftercareSvc)
	roadsideCustomerReportHandler := handler.NewRoadsideCustomerReportHandler(serviceReportSvc, serviceAdjustmentSvc)
	...
	mux.HandleFunc("/api/v1/admin/roadside/settlements/finalize", middleware.BaseChain(middleware.AuthMiddleware(middleware.RequireIdempotencyKey(writeDB, "roadside.settlement.finalize", roadsideSettlementHandler.Finalize))))
	mux.HandleFunc("/api/v1/customer/roadside/adjustments/collect", middleware.BaseChain(middleware.AuthMiddleware(middleware.RequireIdempotencyKey(writeDB, "roadside.adjustment.collect", roadsideCollectionHandler.Start))))
	mux.HandleFunc("/api/v1/customer/roadside/final-report", middleware.BaseChain(middleware.AuthMiddleware(roadsideCustomerReportHandler.GetTambalBanFinalReport)))
	mux.HandleFunc("/api/v1/customer/roadside/claims", middleware.BaseChain(middleware.AuthMiddleware(middleware.RequireIdempotencyKey(writeDB, "roadside.claim.submit", roadsideAftercareHandler.SubmitClaim))))
	mux.HandleFunc("/api/v1/customer/roadside/ratings", middleware.BaseChain(middleware.AuthMiddleware(middleware.RequireIdempotencyKey(writeDB, "roadside.rating.submit", roadsideAftercareHandler.SubmitRating))))
	...
	mux.HandleFunc("/api/v1/customer/service-report/towing", middleware.BaseChain(middleware.AuthMiddleware(roadsideCustomerReportHandler.GetTowingReport)))
